...
* Update translations, see [translation_process.md](https://github.com/AdvancedTechnologyCoin/arc_core/blob/master/doc/translation_process.md#synchronising-translations).

* Update manpages, see [gen-manpages.sh](https://github.com/AdvancedTechnologyCoin/arc_core/blob/master/contrib/devtools/README.md#gen-manpagessh).
...
* Update hardcoded [seeds](/contrib/seeds/README.md). TODO: Give example PR for Arc
...
Check out the source code in the following directory hierarchy.
	...
	git clone https://github.com/arcpay/gitian.sigs.git
	git clone https://github.com/AdvancedTechnologyCoin/arc_core-detached-sigs.git
	git clone https://github.com/devrandom/gitian-builder.git
	git clone https://github.com/AdvancedTechnologyCoin/arc_core.git

### Arc Core maintainers/release engineers, update (commit) version in sources
...
Write release notes. git shortlog helps a lot, for example:
    ...
    git shortlog --no-merges v(current version, e.g. 0.12.8)..v(new version, e.g. 0.12.9)
...
Tag version (or release candidate) in git
    ...
    git tag -s v(new version, e.g. 0.12.9)
...
Setup Gitian descriptors:
    ...
    export VERSION=(new version, e.g. 0.12.9)
    git fetch
...
### Build and sign Arc Core for Linux, Windows, and OS X:

    pushd ./gitian-builder
    ./bin/gbuild --memory 3000 --commit arc=v${VERSION} ../arc/contrib/gitian-descriptors/gitian-linux.yml
    ...
    ./bin/gbuild --memory 3000 --commit arc=v${VERSION} ../arc/contrib/gitian-descriptors/gitian-win.yml
    ...
    ./bin/gbuild --memory 3000 --commit arc=v${VERSION} ../arc/contrib/gitian-descriptors/gitian-osx.yml
...
Add other gitian builders keys to your gpg keyring, and/or refresh keys.
    ...
    gpg --import arc/contrib/gitian-keys/*.pgp
    gpg --refresh-keys
...
- Detached signatures will then be committed to the [arc-detached-sigs](https://github.com/AdvancedTechnologyCoin/arc_core-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.
...
```
arc-${VERSION}-aarch64-linux-gnu.tar.gz
arc-${VERSION}-arm-linux-gnueabihf.tar.gz
arc-${VERSION}-i686-pc-linux-gnu.tar.gz
arc-${VERSION}-x86_64-linux-gnu.tar.gz
arc-${VERSION}-osx64.tar.gz
arc-${VERSION}-osx.dmg
arc-${VERSION}.tar.gz
arc-${VERSION}-win32-setup.exe
arc-${VERSION}-win32.zip
arc-${VERSION}-win64-setup.exe
arc-${VERSION}-win64.zip
```
The `*-debug*` files generated by the gitian build contain debug symbols
for troubleshooting by developers. It is assumed that anyone that is interested
in debugging can run gitian to generate the files for themselves. To avoid
end-user confusion about which file to pick, as well as save storage
space *do not upload these to the arc.org server*.
...
- Announce the release:
  ...
  - Release on Arc forum: https://www.arc.org/forum/topic/official-announcements.54/

  - Optionally Discord, twitter, reddit /r/Arcpay, ... but this will usually sort out itself

  - Notify flare so that he can start building [the PPAs](https://launchpad.net/~arc.org/+archive/ubuntu/arc)

  - Archive release notes for the new version to `doc/release-notes/` (branch `master` and branch of the release)

  - Create a [new GitHub release](https://github.com/AdvancedTechnologyCoin/arc_core/releases/new) with a link to the archived release notes.
