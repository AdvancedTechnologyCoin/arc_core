...
in the Arc Core repository under 'Fetch and create inputs' to install sources which require
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/arc/.git/
...
For example:
```bash
URL=https://github.com/crowning-/arc.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit arc=${COMMIT} --url arc=${URL} ../arc/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit arc=${COMMIT} --url arc=${URL} ../arc/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit arc=${COMMIT} --url arc=${URL} ../arc/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the arc git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../arc/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/arc
SIGPATH=/some/root/path/arc-detached-sigs

./bin/gbuild --url arc=${BTCPATH},signature=${SIGPATH} ../arc/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/arc-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/arc-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/arc-osx-build.assert
```
...
[arc/gitian.sigs](https://github.com/arcpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
