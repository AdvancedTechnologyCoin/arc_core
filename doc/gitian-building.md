...
*Setup instructions for a Gitian build of Arctic Core using a Debian VM or physical system.*
...
- [Building Arctic Core](#building-arcticcoin-core)
...
Clone the git repositories for Arctic Core and Gitian.
...
in the Arctic Core repository under 'Fetch and build inputs' to install sources which require
...
To build Arctic Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Arctic Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/arcticcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/ArcticCore/arcticcoin
...
For example:
```bash
URL=https://github.com/crowning-/arcticcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit arcticcoin=${COMMIT} --url arcticcoin=${URL} ../arcticcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit arcticcoin=${COMMIT} --url arcticcoin=${URL} ../arcticcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit arcticcoin=${COMMIT} --url arcticcoin=${URL} ../arcticcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the arcticcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../arcticcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/arcticcoin.git
SIGPATH=/some/root/path/arcticcoin-detached-sigs.git

./bin/gbuild --url arcticcoin=${BTCPATH},signature=${SIGPATH} ../arcticcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/arcticcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/arcticcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/arcticcoin-osx-build.assert
```
