...
After exiting, you'll get a warning that the install is keg-only, which means it wasn't symlinked to `/usr/local`.  You don't need it to link it to build arcticcoin, but if you want to, here's how:

    $ brew link --force berkeley-db4


### Building `arcticcoind`
...
If you are building `arcticcoind` or `Arctic-Qt` for others, your build machine should be set up
...
Once dependencies are compiled, see release-process.md for how the Arctic-Qt.app
...
commands:
    ...
    echo -e "rpcuser=arcticcoinrpc\nrpcpassword=$(xxd -l 16 -p /dev/urandom)" > "/Users/${USER}/Library/Application Support/Arctic/arcticcoin.conf"
    chmod 600 "/Users/${USER}/Library/Application Support/Arctic/arcticcoin.conf"

The next time you run it, it will start downloading the blockchain, but it won't
output anything while it's doing this. This process may take several hours;
you can monitor its process by looking at the debug.log file, like this:

    tail -f $HOME/Library/Application\ Support/Arctic/debug.log
