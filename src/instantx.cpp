...
// step 2) Top COutPointLock::SIGNATURES_TOTAL goldminenodes per each spent outpoint push "txvote" message
...
void CInstantSend::Vote(CTxLockCandidate& txLockCandidate, CConnman& connman)
{
    ...
    while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
        ...
        int nRank;
        if(!mnodeman.GetGoldminenodeRank(activeGoldminenode.outpoint, nRank, nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION)) {
...
bool CInstantSend::ProcessTxLockVote(CNode* pfrom, CTxLockVote& vote, CConnman& connman)
{
    ...
    if(it == mapTxLockCandidates.end() || !it->second.txLockRequest) {
        if(!mapTxLockVotesOrphan.count(vote.GetHash())) {
            ...
            LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- Orphan vote: txid=%s  goldminenode=%s new\n",
                    txHash.ToString(), vote.GetGoldminenodeOutpoint().ToStringShort());
            ...
            LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- Orphan vote: txid=%s  goldminenode=%s seen\n",
                    txHash.ToString(), vote.GetGoldminenodeOutpoint().ToStringShort());
        }
        ...
        if(!mapGoldminenodeOrphanVotes.count(vote.GetGoldminenodeOutpoint())) {
            mapGoldminenodeOrphanVotes[vote.GetGoldminenodeOutpoint()] = nGoldminenodeOrphanExpireTime;
        } else {
            int64_t nPrevOrphanVote = mapGoldminenodeOrphanVotes[vote.GetGoldminenodeOutpoint()];
            if(nPrevOrphanVote > GetTime() && nPrevOrphanVote > GetAverageGoldminenodeOrphanVoteTime()) {
                LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- goldminenode is spamming orphan Transaction Lock Votes: txid=%s  goldminenode=%s\n",
                        txHash.ToString(), vote.GetGoldminenodeOutpoint().ToStringShort());
        ...
        BOOST_FOREACH(const uint256& hash, it1->second) {
            if(hash != txHash) {
                ...
                if(it2 !=mapTxLockCandidates.end() && it2->second.HasGoldminenodeVoted(vote.GetOutpoint(), vote.GetGoldminenodeOutpoint())) {
                    ...
                    LogPrintf("CInstantSend::ProcessTxLockVote -- goldminenode sent conflicting votes! %s\n", vote.GetGoldminenodeOutpoint().ToStringShort());
...
bool CTxLockVote::IsValid(CNode* pnode, CConnman& connman) const
{
    ...
    int nRank;
    if(!mnodeman.GetGoldminenodeRank(outpointGoldminenode, nRank, nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION)) {
