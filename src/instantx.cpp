...
// step 2) Top COutPointLock::SIGNATURES_TOTAL goldminenodes per each spent outpoint push "txlockvote" message (txlvote)
...
void CInstantSend::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (strCommand == NetMsgType::TXLOCKVOTE) // InstantSend Transaction Lock Consensus Votes
    {
        ...
        // Ignore any InstantSend messages until goldminenode list is synced
...
bool CInstantSend::ProcessTxLockRequest(const CTxLockRequest& txLockRequest, CConnman& connman)
{
    ...
    // Goldminenodes will sometimes propagate votes before the transaction is known to the client.
...
void CInstantSend::Vote(CTxLockCandidate& txLockCandidate, CConnman& connman)
{
    ...
    while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
        ...
        int nRank;
        int nMinRequiredProtocol = std::max(MIN_INSTANTSEND_PROTO_VERSION, mnpayments.GetMinGoldminenodePaymentsProto());
        if(!mnodeman.GetGoldminenodeRank(activeGoldminenode.outpoint, nRank, nLockInputHeight, nMinRequiredProtocol)) {
            LogPrint("instantsend", "CInstantSend::Vote -- Can't calculate rank for goldminenode %s\n", activeGoldminenode.outpoint.ToStringShort());
            ++itOutpointLock;
            continue;
        }
        ...
        if(nRank > nSignaturesTotal) {
            LogPrint("instantsend", "CInstantSend::Vote -- Goldminenode not in the top %d (%d)\n", nSignaturesTotal, nRank);
            ++itOutpointLock;
            continue;
        }
        ...
        if(itVoted != mapVotedOutpoints.end()) {
            for (const auto& hash : itVoted->second) {
                ...
                if(it2->second.HasGoldminenodeVoted(itOutpointLock->first, activeGoldminenode.outpoint)) {
        ...
        CTxLockVote vote(txHash, itOutpointLock->first, activeGoldminenode.outpoint);
...
bool CInstantSend::ProcessNewTxLockVote(CNode* pfrom, const CTxLockVote& vote, CConnman& connman)
{
    ...
    if(it == mapTxLockCandidates.end() || !it->second.txLockRequest) {
        ...
        LogPrint("instantsend", "CInstantSend::%s -- Orphan vote: txid=%s  goldminenode=%s %s\n",
                __func__, txHash.ToString(), vote.GetGoldminenodeOutpoint().ToStringShort(), fInserted ? "new" : "seen");
        ...
        int nGoldminenodeOrphanExpireTime = GetTime() + 60*10; // keep time data for 10 minutes
        auto itMnOV = mapGoldminenodeOrphanVotes.find(vote.GetGoldminenodeOutpoint());
        if(itMnOV == mapGoldminenodeOrphanVotes.end()) {
            mapGoldminenodeOrphanVotes.emplace(vote.GetGoldminenodeOutpoint(), nGoldminenodeOrphanExpireTime);
        } else {
            if(itMnOV->second > GetTime() && itMnOV->second > GetAverageGoldminenodeOrphanVoteTime()) {
                LogPrint("instantsend", "CInstantSend::%s -- goldminenode is spamming orphan Transaction Lock Votes: txid=%s  goldminenode=%s\n",
                        __func__, txHash.ToString(), vote.GetGoldminenodeOutpoint().ToStringShort());
                // Misbehaving(pfrom->id, 1);
                return false;
            }
            // not spamming, refresh
            itMnOV->second = nGoldminenodeOrphanExpireTime;
        }
    ...
    if(it1 != mapVotedOutpoints.end()) {
        for (const auto& hash : it1->second) {
            if(hash != txHash) {
                ...
                // let's see if it was the same goldminenode who voted on this outpoint
                // for another tx lock request
                std::map<uint256, CTxLockCandidate>::iterator it2 = mapTxLockCandidates.find(hash);
                if(it2 !=mapTxLockCandidates.end() && it2->second.HasGoldminenodeVoted(vote.GetOutpoint(), vote.GetGoldminenodeOutpoint())) {
                    // yes, it was the same goldminenode
                    LogPrintf("CInstantSend::%s -- goldminenode sent conflicting votes! %s\n", __func__, vote.GetGoldminenodeOutpoint().ToStringShort());
                    ...
                    // apply maximum PoSe ban score to this goldminenode i.e. PoSe-ban it instantly
                    mnodeman.PoSeBan(vote.GetGoldminenodeOutpoint());
                    // NOTE: This vote must be relayed further to let all other nodes know about such
                    // misbehaviour of this goldminenode. This way they should also be able to construct
                    // conflicting lock and PoSe-ban this goldminenode.
                }
            }
        }
        // store all votes, regardless of them being sent by malicious goldminenode or not
...
bool CInstantSend::ResolveConflicts(const CTxLockCandidate& txLockCandidate)
{
    ...
    for (const auto& txin : txLockCandidate.txLockRequest.tx->vin) {
        ...
        if(GetLockedOutPointTxHash(txin.prevout, hashConflicting) && txHash != hashConflicting) {
            ...
            //       (not a big issue since we already PoSe ban malicious goldminenodes
            //        and they won't be able to spam)
            // TODO: ban all malicious goldminenodes permanently, do not accept anything from them, ever
...
int64_t CInstantSend::GetAverageGoldminenodeOrphanVoteTime()
{
    LOCK(cs_instantsend);
    // NOTE: should never actually call this function when mapGoldminenodeOrphanVotes is empty
    if(mapGoldminenodeOrphanVotes.empty()) return 0;

    std::map<COutPoint, int64_t>::iterator it = mapGoldminenodeOrphanVotes.begin();
    int64_t total = 0;

    while(it != mapGoldminenodeOrphanVotes.end()) {
        total+= it->second;
        ++it;
    }

    return total / mapGoldminenodeOrphanVotes.size();
}
...
void CInstantSend::CheckAndRemove()
{
    ...
    while(itVote != mapTxLockVotes.end()) {
        if(itVote->second.IsExpired(nCachedBlockHeight)) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired vote: txid=%s  goldminenode=%s\n",
                    itVote->second.GetTxHash().ToString(), itVote->second.GetGoldminenodeOutpoint().ToStringShort());
    ...
    while(itOrphanVote != mapTxLockVotesOrphan.end()) {
        if(itOrphanVote->second.IsTimedOut()) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing timed out orphan vote: txid=%s  goldminenode=%s\n",
                    itOrphanVote->second.GetTxHash().ToString(), itOrphanVote->second.GetGoldminenodeOutpoint().ToStringShort());
    ...
    while(itVote != mapTxLockVotes.end()) {
        if(itVote->second.IsFailed()) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing vote for failed lock attempt: txid=%s  goldminenode=%s\n",
                    itVote->second.GetTxHash().ToString(), itVote->second.GetGoldminenodeOutpoint().ToStringShort());
            mapTxLockVotes.erase(itVote++);
        } else {
            ++itVote;
    ...
    // remove timed out goldminenode orphan votes (DOS protection)
    std::map<COutPoint, int64_t>::iterator itGoldminenodeOrphan = mapGoldminenodeOrphanVotes.begin();
    while(itGoldminenodeOrphan != mapGoldminenodeOrphanVotes.end()) {
        if(itGoldminenodeOrphan->second < GetTime()) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing timed out orphan goldminenode vote: goldminenode=%s\n",
                    itGoldminenodeOrphan->first.ToStringShort());
            mapGoldminenodeOrphanVotes.erase(itGoldminenodeOrphan++);
        } else {
            ++itGoldminenodeOrphan;
        }
    }
...
{
    if(!mnodeman.Has(outpointGoldminenode)) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- Unknown goldminenode %s\n", outpointGoldminenode.ToStringShort());
        mnodeman.AskForMN(pnode, outpointGoldminenode, connman);
        return false;
    }
    ...
    int nRank;
    int nMinRequiredProtocol = std::max(MIN_INSTANTSEND_PROTO_VERSION, mnpayments.GetMinGoldminenodePaymentsProto());
    if(!mnodeman.GetGoldminenodeRank(outpointGoldminenode, nRank, nLockInputHeight, nMinRequiredProtocol)) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("instantsend", "CTxLockVote::IsValid -- Can't calculate rank for goldminenode %s\n", outpointGoldminenode.ToStringShort());
        return false;
    }
    LogPrint("instantsend", "CTxLockVote::IsValid -- Goldminenode %s, rank=%d\n", outpointGoldminenode.ToStringShort(), nRank);

    int nSignaturesTotal = COutPointLock::SIGNATURES_TOTAL;
    if(nRank > nSignaturesTotal) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- Goldminenode %s is not in the top %d (%d), vote hash=%s\n",
                outpointGoldminenode.ToStringShort(), nSignaturesTotal, nRank, GetHash().ToString());
        return false;
    }
...
bool CTxLockVote::CheckSignature() const
{
    ...
    if(!mnodeman.GetGoldminenodeInfo(outpointGoldminenode, infoMn)) {
        LogPrintf("CTxLockVote::CheckSignature -- Unknown Goldminenode: goldminenode=%s\n", outpointGoldminenode.ToString());
        return false;
    }
    ...
    if (sporkManager.IsSporkActive(SPORK_6_NEW_SIGS)) {
        ...
        if (!CHashSigner::VerifyHash(hash, infoMn.pubKeyGoldminenode, vchGoldminenodeSignature, strError)) {
            // could be a signature in old format
            std::string strMessage = txHash.ToString() + outpoint.ToStringShort();
            if(!CMessageSigner::VerifyMessage(infoMn.pubKeyGoldminenode, vchGoldminenodeSignature, strMessage, strError)) {
...
bool CTxLockVote::Sign()
{
    ...
    if (sporkManager.IsSporkActive(SPORK_6_NEW_SIGS)) {
        ...
        if(!CHashSigner::SignHash(hash, activeGoldminenode.keyGoldminenode, vchGoldminenodeSignature)) {
            LogPrintf("CTxLockVote::Sign -- SignHash() failed\n");
            return false;
        }

        if (!CHashSigner::VerifyHash(hash, activeGoldminenode.pubKeyGoldminenode, vchGoldminenodeSignature, strError)) {
        ...
        if(!CMessageSigner::SignMessage(strMessage, vchGoldminenodeSignature, activeGoldminenode.keyGoldminenode)) {
            LogPrintf("CTxLockVote::Sign -- SignMessage() failed\n");
            return false;
        }

        if(!CMessageSigner::VerifyMessage(activeGoldminenode.pubKeyGoldminenode, vchGoldminenodeSignature, strMessage, strError)) {
...
{
    if(mapGoldminenodeVotes.count(vote.GetGoldminenodeOutpoint()))
        return false;
    mapGoldminenodeVotes.insert(std::make_pair(vote.GetGoldminenodeOutpoint(), vote));
    return true;
}
...
std::vector<CTxLockVote> COutPointLock::GetVotes() const
{
    ...
    std::map<COutPoint, CTxLockVote>::const_iterator itVote = mapGoldminenodeVotes.begin();
    while(itVote != mapGoldminenodeVotes.end()) {
...
bool COutPointLock::HasGoldminenodeVoted(const COutPoint& outpointGoldminenodeIn) const
{
    return mapGoldminenodeVotes.count(outpointGoldminenodeIn);
}
...
bool CTxLockCandidate::HasGoldminenodeVoted(const COutPoint& outpointIn, const COutPoint& outpointGoldminenodeIn)
{
    std::map<COutPoint, COutPointLock>::iterator it = mapOutPointLocks.find(outpointIn);
    return it !=mapOutPointLocks.end() && it->second.HasGoldminenodeVoted(outpointGoldminenodeIn);
}
