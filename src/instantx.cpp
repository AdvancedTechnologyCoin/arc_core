...
void CInstantSend::Vote(CTxLockCandidate& txLockCandidate)
{
    ...
    while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
        ...
        int n = mnodeman.GetGoldminenodeRank(activeGoldminenode.vin, nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION);

        if(n == -1) {
            LogPrint("instantsend", "CInstantSend::Vote -- Unknown Goldminenode %s\n", activeGoldminenode.vin.prevout.ToStringShort());
            ++itOutpointLock;
            continue;
        }
        ...
        if(itVoted != mapVotedOutpoints.end()) {
            BOOST_FOREACH(const uint256& hash, itVoted->second) {
                ...
                if(it2->second.HasGoldminenodeVoted(itOutpointLock->first, activeGoldminenode.vin.prevout)) {
        ...
        CTxLockVote vote(txHash, itOutpointLock->first, activeGoldminenode.vin.prevout);
    ...
    if(it1 != mapVotedOutpoints.end()) {
        BOOST_FOREACH(const uint256& hash, it1->second) {
            if(hash != txHash) {
                ...
                std::map<uint256, CTxLockCandidate>::iterator it2 = mapTxLockCandidates.find(hash);
                if(it2->second.HasGoldminenodeVoted(vote.GetOutpoint(), vote.GetGoldminenodeOutpoint())) {
                    // yes, it did, refuse to accept a vote to include the same outpoint in another tx
                    // from the same goldminenode.
                    // TODO: apply pose ban score to this goldminenode?
        ...
        // we have votes by other goldminenodes only (so far), let's continue and see who will win
...
void CInstantSend::CheckAndRemove()
{
    ...
    while(itOrphanVote != mapTxLockVotesOrphan.end()) {
        if(GetTime() - itOrphanVote->second.GetTimeCreated() > ORPHAN_VOTE_SECONDS) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired orphan vote: txid=%s  goldminenode=%s\n",
    ...
    // remove expired goldminenode orphan votes (DOS protection)
    ...
    while(itGoldminenodeOrphan != mapGoldminenodeOrphanVotes.end()) {
        if(itGoldminenodeOrphan->second < GetTime()) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired orphan goldminenode vote: goldminenode=%s\n",
...
bool CTxLockVote::IsValid(CNode* pnode) const
{
    ...
    int n = mnodeman.GetGoldminenodeRank(CTxIn(outpointGoldminenode), nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION);

    if(n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("instantsend", "CTxLockVote::IsValid -- Outdated goldminenode %s\n", outpointGoldminenode.ToStringShort());
        return false;
    }
    LogPrint("instantsend", "CTxLockVote::IsValid -- Goldminenode %s, rank=%d\n", outpointGoldminenode.ToStringShort(), n);

    int nSignaturesTotal = COutPointLock::SIGNATURES_TOTAL;
    if(n > nSignaturesTotal) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- Goldminenode %s is not in the top %d (%d), vote hash=%s\n",
                outpointGoldminenode.ToStringShort(), nSignaturesTotal, n, GetHash().ToString());
        return false;
    }
...
bool CTxLockVote::CheckSignature() const
{
    ...
    goldminenode_info_t infoMn = mnodeman.GetGoldminenodeInfo(CTxIn(outpointGoldminenode));
    ...
    if(!darkSendSigner.VerifyMessage(infoMn.pubKeyGoldminenode, vchGoldminenodeSignature, strMessage, strError)) {
...
bool CTxLockVote::Sign()
{
    ...
    if(!darkSendSigner.SignMessage(strMessage, vchGoldminenodeSignature, activeGoldminenode.keyGoldminenode)) {
        LogPrintf("CTxLockVote::Sign -- SignMessage() failed\n");
        return false;
    }

    if(!darkSendSigner.VerifyMessage(activeGoldminenode.pubKeyGoldminenode, vchGoldminenodeSignature, strMessage, strError)) {
