...
void PrepareShutdown()
{
    ...
    if (!fLiteMode) {
        CFlatDB<CGoldminenodeMan> flatdb1("mncache.dat", "magicGoldminenodeCache");
        flatdb1.Dump(mnodeman);
        CFlatDB<CGoldminenodePayments> flatdb2("mnpayments.dat", "magicGoldminenodePaymentsCache");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Arc specific functionality (Goldminenodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf(_("Specify goldminenode configuration file (default: %s)"), "goldminenode.conf"));
    strUsage += HelpMessageOpt("-mnconflock=<n>", strprintf(_("Lock goldminenodes from goldminenode configuration file (default: %u)"), 1));
    strUsage += HelpMessageOpt("-goldminenodeprivkey=<n>", _("Set the goldminenode private key"));
    ...
    strUsage += HelpMessageOpt("-privatesendrounds=<n>", strprintf(_("Use N separate goldminenodes for each denominated input to mix funds (%u-%u, default: %u)"), MIN_PRIVATESEND_ROUNDS, MAX_PRIVATESEND_ROUNDS, DEFAULT_PRIVATESEND_ROUNDS));
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N ARC anonymized (%u-%u, default: %u)"), MIN_PRIVATESEND_AMOUNT, MAX_PRIVATESEND_AMOUNT, DEFAULT_PRIVATESEND_AMOUNT));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/AdvancedTechnologyCoin/arc_core>";
    const std::string URL_WEBSITE = "<https://arc.org>";
...
void InitParameterInteraction()
{
    ...
    if (GetBoolArg("-goldminenode", false)) {
        // goldminenodes MUST accept connections from outside
        ForceSetArg("-listen", "1");
        LogPrintf("%s: parameter interaction: -goldminenode=1 -> setting -listen=1\n", __func__);
        if (GetArg("-maxconnections", DEFAULT_MAX_PEER_CONNECTIONS) < DEFAULT_MAX_PEER_CONNECTIONS) {
            // goldminenodes MUST be able to handle at least DEFAULT_MAX_PEER_CONNECTIONS connections
            ForceSetArg("-maxconnections", itostr(DEFAULT_MAX_PEER_CONNECTIONS));
            LogPrintf("%s: parameter interaction: -goldminenode=1 -> setting -maxconnections=%d\n", __func__, DEFAULT_MAX_PEER_CONNECTIONS);
        }
    }
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    fGoldminenodeMode = GetBoolArg("-goldminenode", false);
    // TODO: goldminenode should have no wallet

    //lite mode disables all Arc-specific functionality
    fLiteMode = GetBoolArg("-litemode", false);

    if(fLiteMode) {
        InitWarning(_("You are starting in lite mode, all Arc-specific functionality is disabled."));
    }

    if((!fLiteMode && fTxIndex == false)
       && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/AdvancedTechnologyCoin/arc_core/pull/1817 and https://github.com/AdvancedTechnologyCoin/arc_core/pull/1743
    ...
    if(fLiteMode && fGoldminenodeMode) {
        return InitError(_("You can not start a goldminenode in lite mode."));
    }
    ...
    if(fGoldminenodeMode) {
        ...
        if(!strMasterNodePrivKey.empty()) {
            if(!CMessageSigner::GetKeysFromSecret(strMasterNodePrivKey, activeGoldminenode.keyGoldminenode, activeGoldminenode.pubKeyGoldminenode))
            ...
            return InitError(_("You must specify a goldminenodeprivkey in the configuration. Please see documentation for help."));
        }
    }

#ifdef ENABLE_WALLET
    LogPrintf("Using goldminenode config file %s\n", GetGoldminenodeConfigFile().string());

    if(GetBoolArg("-mnconflock", true) && pwalletMain && (goldminenodeConfig.getCount() > 0)) {
        LOCK(pwalletMain->cs_wallet);
        LogPrintf("Locking Goldminenodes:\n");
        uint256 mnTxHash;
    ...
    if (!fLiteMode) {
        ...
        if(mnodeman.size()) {
            ...
            CFlatDB<CGoldminenodePayments> flatdb2(strDBName, "magicGoldminenodePaymentsCache");
