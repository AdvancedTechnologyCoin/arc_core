...
bool CDarksendQueue::Sign()
{
    ...
    if (sporkManager.IsSporkActive(SPORK_6_NEW_SIGS)) {
        ...
        std::string strMessage = CTxIn(goldminenodeOutpoint).ToString() +
...
bool CDarksendQueue::CheckSignature(const CPubKey& pubKeyGoldminenode) const
{
    std::string strError = "";
    ...
    if (sporkManager.IsSporkActive(SPORK_6_NEW_SIGS)) {
        ...
        if(!CMessageSigner::VerifyMessage(pubKeyGoldminenode, vchSig, strMessage, strError)) {
            LogPrintf("CDarksendQueue::CheckSignature -- Got bad Goldminenode queue signature: %s; error: %s\n", ToString(), strError);
            return false;
        }
    }
...
bool CDarksendBroadcastTx::CheckSignature(const CPubKey& pubKeyGoldminenode) const
{
    std::string strError = "";
...
bool CPrivateSend::IsCollateralValid(const CTransaction& txCollateral)
{
    ...
    for (const auto& txout : txCollateral.vout) {
        ...
        bool fAllowData = mnpayments.GetMinGoldminenodePaymentsProto() > 70209;
...
{
    if (mnpayments.GetMinGoldminenodePaymentsProto() > 70209) {
...
std::string CPrivateSend::GetMessageByID(PoolMessage nMessageID)
{
    switch (nMessageID) {
        ...
        case ERR_NOT_A_MN:              return _("This is not a Goldminenode."); // not used
...
{
    if(pindex && !fLiteMode && goldminenodeSync.IsGoldminenodeListSynced()) {
...
void ThreadCheckPrivateSend(CConnman& connman)
{
    ...
    while (true)
    {
        ...
        goldminenodeSync.ProcessTick(connman);
        ...
        if(goldminenodeSync.IsBlockchainSynced() && !ShutdownRequested()) {
            ...
            if(nTick % 60 == 0) {
                ...
                mnodeman.ProcessGoldminenodeConnections(connman);
                mnodeman.CheckAndRemove(connman);
                mnodeman.WarnGoldminenodeDaemonUpdates();
                mnpayments.CheckAndRemove();
                instantsend.CheckAndRemove();
            }
            if(fGoldminenodeMode && (nTick % (60 * 5) == 0)) {
