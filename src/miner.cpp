...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const CScript& scriptPubKeyIn)
{
...

    {
        ...
        // Update coinbase transaction with additional info about goldminenode,
        // get some info back to pass to getblocktemplate
        FillBlockPayments(txNew, nHeight, blockReward, blockEvolution, pblock->txoutGoldminenode, pblock->voutSuperblock);
        // LogPrintf("CreateNewBlock -- nBlockHeight %d blockReward %lld txoutGoldminenode %s txNew %s",
        //             nHeight, blockReward, pblock->txoutGoldminenode.ToString(), txNew.ToString());
...
// ***TODO*** ScanHash is not yet used in Arc
//
...
{
    LogPrintf("ArcMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("arc-miner");
        ...
        while (true) {
            if (chainparams.MiningRequiresPeers()) {
                ...
                do {
                    ...
                    if (!fvNodesEmpty && !IsInitialBlockDownload() && goldminenodeSync.IsSynced())
                        break;
                    MilliSleep(1000);
                } while (true);
            }
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("ArcMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("ArcMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("ArcMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("ArcMiner -- runtime error: %s\n", e.what());
        return;
    }
}
