...
/*
    At 15 signatures, 1/2 of the goldminenode network can be owned by
...
private:
    ...
    /// Track goldminenodes who voted with no txlockrequest (for DOS protection)
...
/**
 * An InstantSend transaction lock vote. Sent by a goldminenode in response to a
 ...
 * locked along with the goldminenodes outpoint and signature.
 * @see CTxLockRequest
 */
class CTxLockVote
{
private:
    uint256 txHash;
    COutPoint outpoint;
    COutPoint outpointGoldminenode;
...
public:
    CTxLockVote() :
        txHash(),
        outpoint(),
        outpointGoldminenode(),
        vchGoldminenodeSignature(),
        nConfirmedHeight(-1),
        nTimeCreated(GetTime())
        {}

    CTxLockVote(const uint256& txHashIn, const COutPoint& outpointIn, const COutPoint& outpointGoldminenodeIn) :
        txHash(txHashIn),
        outpoint(outpointIn),
        outpointGoldminenode(outpointGoldminenodeIn),
        vchGoldminenodeSignature(),
        nConfirmedHeight(-1),
        nTimeCreated(GetTime())
        {}
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        READWRITE(txHash);
        READWRITE(outpoint);
        READWRITE(outpointGoldminenode);
        if (!(s.GetType() & SER_GETHASH)) {
            READWRITE(vchGoldminenodeSignature);
        }
    }
...
public:
    ...
    bool HasGoldminenodeVoted(const COutPoint& outpointIn, const COutPoint& outpointGoldminenodeIn);
