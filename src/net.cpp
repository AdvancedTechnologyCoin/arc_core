...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Arc users should be well used to small numbers with many decimal places by now :)
...
void CConnman::AcceptConnection(const ListenSocket& hListenSocket) {
    ...
    if(fGoldminenodeMode && !goldminenodeSync.IsSynced()) {
    ...
    while (!interruptNet)
    {
        ...
        //
        {
            ...
            BOOST_FOREACH(CNode* pnode, vNodesCopy)
            {
                ...
                {
                    LogPrintf("ThreadSocketHandler -- removing node: peer=%d addr=%s nRefCount=%d fInbound=%d fGoldminenode=%d\n",
                              pnode->id, pnode->addr.ToString(), pnode->GetRefCount(), pnode->fInbound, pnode->fGoldminenode);
                    ...
                    pnode->grantGoldminenodeOutbound.Release();
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "Arc Core " + FormatFullVersion();
...
void CConnman::ThreadOpenConnections()
{
    ...
    while (!interruptNet)
    {
        ...
        if (!Params().AllowMultipleAddressesFromGroup()) {
            LOCK(cs_vNodes);
            BOOST_FOREACH(CNode* pnode, vNodes) {
                if (!pnode->fInbound && !pnode->fAddnode && !pnode->fGoldminenode) {
...
void CConnman::ThreadOpenGoldminenodeConnections()
{
    // Connecting to specific addresses, no goldminenode connections available
    ...
    while (!interruptNet)
    {
        ...
        CSemaphoreGrant grant(*semGoldminenodeOutbound);
        if (interruptNet)
            return;

        // NOTE: Process only one pending goldminenode at a time

        LOCK(cs_vPendingGoldminenodes);
        if (vPendingGoldminenodes.empty()) {
            // nothing to do, keep waiting
            continue;
        }

        const CService addr = vPendingGoldminenodes.front();
        vPendingGoldminenodes.erase(vPendingGoldminenodes.begin());
        if (IsGoldminenodeOrDisconnectRequested(addr)) {
            // nothing to do, try the next one
            continue;
        }

        OpenGoldminenodeConnection(CAddress(addr, NODE_NETWORK));
        // should be in the list now if connection was opened
        ForNode(addr, CConnman::AllNodes, [&](CNode* pnode) {
            if (pnode->fDisconnect) {
                return false;
            }
            grant.MoveTo(pnode->grantGoldminenodeOutbound);
            return true;
        });
    }
}
...
bool CConnman::OpenNetworkConnection(const CAddress& addrConnect, bool fCountFailure, CSemaphoreGrant *grantOutbound, const char *pszDest, bool fOneShot, bool fFeeler, bool fAddnode, bool fConnectToGoldminenode)
{
    //
    ...
    if (fAddnode)
        pnode->fAddnode = true;
    if (fConnectToGoldminenode)
        pnode->fGoldminenode = true;
...
bool CConnman::OpenGoldminenodeConnection(const CAddress &addrConnect) {
...
bool CConnman::Start(CScheduler& scheduler, std::string& strNodeError, Options connOptions)
{
    ...
    if (semGoldminenodeOutbound == NULL) {
        // initialize semaphore
        semGoldminenodeOutbound = new CSemaphore(MAX_OUTBOUND_GOLDMINENODE_CONNECTIONS);
    }
    ...
    // Initiate goldminenode connections
    threadOpenGoldminenodeConnections = std::thread(&TraceThread<std::function<void()> >, "mncon", std::function<void()>(std::bind(&CConnman::ThreadOpenGoldminenodeConnections, this)));
...
void CConnman::Interrupt()
{
    ...
    if (semGoldminenodeOutbound) {
        for (int i=0; i<MAX_OUTBOUND_GOLDMINENODE_CONNECTIONS; i++) {
            semGoldminenodeOutbound->post();
        }
    }
}
...
void CConnman::Stop()
{
    ...
    if (threadOpenGoldminenodeConnections.joinable())
        threadOpenGoldminenodeConnections.join();
    ...
    delete semAddnode;
    semAddnode = NULL;
    delete semGoldminenodeOutbound;
    semGoldminenodeOutbound = NULL;
}
...
bool CConnman::AddPendingGoldminenode(const CService& service)
{
    LOCK(cs_vPendingGoldminenodes);
    for(std::vector<CService>::const_iterator it = vPendingGoldminenodes.begin(); it != vPendingGoldminenodes.end(); ++it) {
        if (service == *it)
            return false;
    }

    vPendingGoldminenodes.push_back(service);
    return true;
}
...
bool CConnman::IsGoldminenodeOrDisconnectRequested(const CService& addr) {
    return ForNode(addr, AllNodes, [](CNode* pnode){
        return pnode->fGoldminenode || pnode->fDisconnect;
    });
}
