...
CNode* CConnman::ConnectNode(CAddress addrConnect, const char *pszDest, bool fConnectToGoldminenode)
{
...
void CConnman::ThreadOpenConnections()
{
    ...
    while (!interruptNet)
    {
        ...
        std::set<std::vector<unsigned char> > setConnected;
        {
            LOCK(cs_vNodes);
            BOOST_FOREACH(CNode* pnode, vNodes) {
                if (!pnode->fInbound && !pnode->fGoldminenode) {
...
bool CConnman::BindListenPort(const CService &addrBind, std::string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Arc Core is probably already running."), addrBind.ToString());
        else
