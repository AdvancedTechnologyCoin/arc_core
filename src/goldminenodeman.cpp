...
bool CMasternodeDB::Write(const CMasternodeMan& mnodemanToSave)
{
    ...
    ssMasternodes << strMagicMessage; // goldmine cache file specific magic message
...
void CMasternodeMan::Remove(CTxIn vin)
{
    ...
    while(it != vMasternodes.end()){
        if((*it).vin == vin){
            LogPrint("goldmine", "CGoldmineMan:: Removing Goldminenode %s - %i now\n", (*it).addr.ToString(), size() - 1);
...
std::string CMasternodeMan::ToString() const
{
    ...
    info << "Goldmines: " << (int)vMasternodes.size() <<
            ", peers who asked us for Goldminenode list: " << (int)mAskedUsForMasternodeList.size() <<
            ", peers we asked for Goldminenode list: " << (int)mWeAskedForMasternodeList.size() <<
            ", entries in Goldminenode list we asked for: " << (int)mWeAskedForMasternodeListEntry.size() <<
