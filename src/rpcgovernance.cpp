...
UniValue gobject(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-alias         - Vote on a governance object by goldminenode alias (using goldminenode.conf setup)\n"
                "  vote-conf          - Vote on a governance object by goldminenode configured in arcticcoin.conf\n"
                "  vote-many          - Vote on a governance object by all goldminenodes (using goldminenode.conf setup)\n"
                );
    ...
    */
    ...
    if(strCommand == "prepare")
    {
        ...
        if((govobj.GetObjectType() == GOVERNANCE_OBJECT_TRIGGER) ||
           (govobj.GetObjectType() == GOVERNANCE_OBJECT_WATCHDOG)) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Trigger and watchdog objects need not be prepared (however only goldminenodes can create them)");
        }
    ...
    if(strCommand == "submit")
    {
        ...
        if(!goldminenodeSync.IsBlockchainSynced()) {
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Must wait for client to sync with goldminenode network. Try again in a minute or so.");
        }

        CGoldminenode mn;
        bool fMnFound = mnodeman.Get(activeGoldminenode.vin, mn);

        DBG( cout << "gobject: submit activeGoldminenode.pubKeyGoldminenode = " << activeGoldminenode.pubKeyGoldminenode.GetHash().ToString()
             << ", vin = " << activeGoldminenode.vin.prevout.ToStringShort()
        ...
        if((govobj.GetObjectType() == GOVERNANCE_OBJECT_TRIGGER) ||
           (govobj.GetObjectType() == GOVERNANCE_OBJECT_WATCHDOG)) {
            if(fMnFound) {
                govobj.SetGoldminenodeInfo(mn.vin);
                govobj.Sign(activeGoldminenode.keyGoldminenode, activeGoldminenode.pubKeyGoldminenode);
            }
            else {
                LogPrintf("gobject(submit) -- Object submission rejected because node is not a goldminenode\n");
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Only valid goldminenodes can submit this type of object");
            }
        }
        else {
            if(params.size() != 6) {
        ...
        if(!governance.GoldminenodeRateCheck(govobj)) {
        ...
        if(!governance.GoldminenodeRateCheck(govobj, true)) {
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            statusObj.push_back(Pair("errorMessage", "Can't find goldminenode by collateral output"));
            resultsObj.push_back(Pair("arcticcoin.conf", statusObj));
        ...
        if(!vote.Sign(activeGoldminenode.keyGoldminenode, activeGoldminenode.pubKeyGoldminenode)) {
            nFailed++;
            statusObj.push_back(Pair("result", "failed"));
    ...
    if(strCommand == "vote-many")
    {
        ...
        BOOST_FOREACH(CGoldminenodeConfig::CGoldminenodeEntry mne, goldminenodeConfig.getEntries()) {
            ...
            if(!darkSendSigner.GetKeysFromSecret(mne.getPrivKey(), keyGoldminenode, pubKeyGoldminenode)){
                nFailed++;
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("errorMessage", "Goldminenode signing error, could not set key correctly"));
            ...
            CGovernanceVote vote(mn.vin, hash, eVoteSignal, eVoteOutcome);
            if(!vote.Sign(keyGoldminenode, pubKeyGoldminenode)){
                nFailed++;
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("errorMessage", "Failure to sign."));
                resultsObj.push_back(Pair(mne.getAlias(), statusObj));
    ...
    // GOLDMINENODES CAN VOTE ON GOVERNANCE OBJECTS ON THE NETWORK FOR VARIOUS SIGNALS AND OUTCOMES
    if(strCommand == "vote-alias")
    {
        ...
        // EXECUTE VOTE FOR EACH GOLDMINENODE, COUNT SUCCESSES VS FAILURES
        ...
        BOOST_FOREACH(CGoldminenodeConfig::CGoldminenodeEntry mne, goldminenodeConfig.getEntries())
        {
            ...
            // SETUP THE SIGNING KEY FROM GOLDMINENODE.CONF ENTRY
            ...
            if(!darkSendSigner.GetKeysFromSecret(mne.getPrivKey(), keyGoldminenode, pubKeyGoldminenode)) {
                nFailed++;
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("errorMessage", strprintf("Invalid goldminenode key %s.", mne.getPrivKey())));
                resultsObj.push_back(Pair(mne.getAlias(), statusObj));
                continue;
            }

            // SEARCH FOR THIS GOLDMINENODE ON THE NETWORK, THE NODE MUST BE ACTIVE TO VOTE
            ...
            if(!fMnFound) {
                nFailed++;
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("errorMessage", "Goldminenode must be publically available on network to vote. Goldminenode not found."));
                resultsObj.push_back(Pair(mne.getAlias(), statusObj));
                continue;
            }
    ...
    if(strCommand == "list" || strCommand == "diff")
    {
        ...
        BOOST_FOREACH(CGovernanceObject* pGovObj, objs)
        {
            ...
            if(goldminenodeVin != CTxIn()) {
                bObj.push_back(Pair("SigningGoldminenode", goldminenodeVin.prevout.ToStringShort()));
            }
    ...
    if(strCommand == "get")
    {
        ...
        const CTxIn& goldminenodeVin = pGovObj->GetGoldminenodeVin();
        if(goldminenodeVin != CTxIn()) {
            objResult.push_back(Pair("SigningGoldminenode", goldminenodeVin.prevout.ToStringShort()));
        }
        ...
        // -- ENDORSED VIA GOLDMINENODE-ELECTED BOARD
    ...
    if(strCommand == "getcurrentvotes")
    {
        ...
        if (params.size() == 4) {
            uint256 txid = ParseHashV(params[2], "Goldminenode Collateral hash");
...
UniValue voteraw(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 7)
        throw std::runtime_error(
                "voteraw <goldminenode-tx-hash> <goldminenode-tx-index> <governance-hash> <vote-signal> [yes|no|abstain] <time> <vote-sig>\n"
    ...
    if(!fMnFound) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failure to find goldminenode in list : " + vin.prevout.ToStringShort());
    }
...
UniValue getgovernanceinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0) {
        throw std::runtime_error(
            ...
            "  \"goldminenodewatchdogmaxseconds\": xxxxx,  (numeric) sentinel watchdog expiration time in seconds\n"
    ...
    obj.push_back(Pair("goldminenodewatchdogmaxseconds", GOLDMINENODE_WATCHDOG_MAX_SECONDS));
