...
void CPrivateSendClient::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(strCommand == NetMsgType::DSQUEUE) {
        ...
        if(!mnodeman.GetGoldminenodeInfo(dsq.vin.prevout, infoMn)) return;
        ...
        if(dsq.fReady) {
            ...
            if(infoMixingGoldminenode.fInfoValid && infoMixingGoldminenode.vin.prevout == dsq.vin.prevout) {
                dsq.fTried = true;
            }
...
void CPrivateSendClient::CheckTimeout()
{
    ...
    if(!fEnablePrivateSend && !fGoldmineNode) return;
...
{
    if(fGoldmineNode) return false; // no client-side mixing on goldminenodes
...
bool CPrivateSendClient::JoinExistingQueue(CAmount nBalanceNeedsAnonymized, CConnman& connman)
{
    ...
    BOOST_FOREACH(CSpysendQueue& dsq, vecSpysendQueue) {
        ...
        if(!mnodeman.GetGoldminenodeInfo(dsq.vin.prevout, infoMn)) {
        ...
        vecGoldminenodesUsed.push_back(dsq.vin.prevout);

        bool fSkip = false;
        connman.ForNode(infoMn.addr, CConnman::AllNodes, [&fSkip](CNode* pnode) {
            fSkip = pnode->fDisconnect || pnode->fGoldminenode;
            return true;
        });
        if (fSkip) {
        ...
        LogPrintf("CPrivateSendClient::JoinExistingQueue -- attempt to connect to goldminenode from queue, addr=%s\n", infoMn.addr.ToString());
...
bool CPrivateSendClient::StartNewQueue(CAmount nValueMin, CAmount nBalanceNeedsAnonymized, CConnman& connman)
{
    ...
    while(nTries < 10) {
        ...
        vecGoldminenodesUsed.push_back(infoMn.vin.prevout);
...
void ThreadCheckPrivateSendClient(CConnman& connman)
{
    ...
    if(fGoldmineNode) return; // no client-side mixing on goldminenodes
