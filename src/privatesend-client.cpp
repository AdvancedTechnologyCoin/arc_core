...
void CPrivateSendClient::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // ignore all Arc related functionality
    ...
    if(strCommand == NetMsgType::DSQUEUE) {
        ...
        if(!mnodeman.GetGoldminenodeInfo(dsq.goldminenodeOutpoint, infoMn)) return;

        if(!dsq.CheckSignature(infoMn.pubKeyGoldminenode)) {
            // we probably have outdated info
            mnodeman.AskForMN(pfrom, dsq.goldminenodeOutpoint, connman);
            return;
        }

        // if the queue is ready, submit if we can
        if(dsq.fReady) {
            if(!infoMixingGoldminenode.fInfoValid) return;
            if(infoMixingGoldminenode.addr != infoMn.addr) {
                LogPrintf("DSQUEUE -- message doesn't match current Goldminenode: infoMixingGoldminenode=%s, addr=%s\n", infoMixingGoldminenode.addr.ToString(), infoMn.addr.ToString());
                return;
            }

            if(nState == POOL_STATE_QUEUE) {
                LogPrint("privatesend", "DSQUEUE -- PrivateSend queue (%s) is ready on goldminenode %s\n", dsq.ToString(), infoMn.addr.ToString());
                SubmitDenominate(connman);
            }
        } else {
            for (const auto& q : vecDarksendQueue) {
                if(q.goldminenodeOutpoint == dsq.goldminenodeOutpoint) {
            ...
            if(infoMn.nLastDsq != 0 && nThreshold > mnodeman.nDsqCount) {
                LogPrint("privatesend", "DSQUEUE -- Goldminenode %s is sending too many dsq messages\n", infoMn.addr.ToString());
                return;
            }

            if(!mnodeman.AllowMixing(dsq.goldminenodeOutpoint)) return;

            LogPrint("privatesend", "DSQUEUE -- new PrivateSend queue (%s) from goldminenode %s\n", dsq.ToString(), infoMn.addr.ToString());
            if(infoMixingGoldminenode.fInfoValid && infoMixingGoldminenode.outpoint == dsq.goldminenodeOutpoint) {
                dsq.fTried = true;
            }
        ...
        if(!infoMixingGoldminenode.fInfoValid) return;
        if(infoMixingGoldminenode.addr != pfrom->addr) {
            //LogPrintf("DSSTATUSUPDATE -- message doesn't match current Goldminenode: infoMixingGoldminenode %s addr %s\n", infoMixingGoldminenode.addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if(!infoMixingGoldminenode.fInfoValid) return;
        if(infoMixingGoldminenode.addr != pfrom->addr) {
            //LogPrintf("DSFINALTX -- message doesn't match current Goldminenode: infoMixingGoldminenode %s addr %s\n", infoMixingGoldminenode.addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if(!infoMixingGoldminenode.fInfoValid) return;
        if(infoMixingGoldminenode.addr != pfrom->addr) {
            LogPrint("privatesend", "DSCOMPLETE -- message doesn't match current Goldminenode: infoMixingGoldminenode=%s  addr=%s\n", infoMixingGoldminenode.addr.ToString(), pfrom->addr.ToString());
            return;
        }
...
void CPrivateSendClient::SetNull()
{
    ...
    infoMixingGoldminenode = goldminenode_info_t();
...
std::string CPrivateSendClient::GetStatus()
{
    ...
    if(WaitForAnotherBlock() || !goldminenodeSync.IsBlockchainSynced())
    ...
    switch(nState) {
        ...
        case POOL_STATE_QUEUE:
            ...
            return strprintf(_("Submitted to goldminenode, waiting in queue %s"), strSuffix);;
        case POOL_STATE_ACCEPTING_ENTRIES:
            if(nEntriesCount == 0) {
                ...
                if(     nStatusMessageProgress % 70 <= 40) return strprintf(_("Submitted following entries to goldminenode: %u / %d"), nEntriesCount, CPrivateSend::GetMaxPoolTransactions());
                else if(nStatusMessageProgress % 70 <= 50) strSuffix = ".";
                else if(nStatusMessageProgress % 70 <= 60) strSuffix = "..";
                else if(nStatusMessageProgress % 70 <= 70) strSuffix = "...";
                return strprintf(_("Submitted to goldminenode, waiting for more entries ( %u / %d ) %s"), nEntriesCount, CPrivateSend::GetMaxPoolTransactions(), strSuffix);
            }
...
bool CPrivateSendClient::GetMixingGoldminenodeInfo(goldminenode_info_t& mnInfoRet)
{
    mnInfoRet = infoMixingGoldminenode.fInfoValid ? infoMixingGoldminenode : goldminenode_info_t();
    return infoMixingGoldminenode.fInfoValid;
}

bool CPrivateSendClient::IsMixingGoldminenode(const CNode* pnode)
{
    return infoMixingGoldminenode.fInfoValid && pnode->addr == infoMixingGoldminenode.addr;
}
...
bool CPrivateSendClient::SendDenominate(const std::vector<CTxDSIn>& vecTxDSIn, const std::vector<CTxOut>& vecTxOut, CConnman& connman)
{
    if(fGoldminenodeMode) {
        LogPrintf("CPrivateSendClient::SendDenominate -- PrivateSend from a Goldminenode is not supported currently.\n");
        return false;
    }
    ...
    if(!nSessionID) {
        LogPrintf("CPrivateSendClient::SendDenominate -- No Goldminenode has been selected yet.\n");
        UnlockCoins();
        keyHolderStorage.ReturnAll();
        SetNull();
        return false;
...
{
    if(fGoldminenodeMode) return false;

    // do not update state when mixing client state is one of these
    if(nState == POOL_STATE_IDLE || nState == POOL_STATE_ERROR || nState == POOL_STATE_SUCCESS) return false;

    strAutoDenomResult = _("Goldminenode:") + " " + CPrivateSend::GetMessageByID(nMessageID);

    // if rejected at any state
    if(nStatusUpdate == STATUS_REJECTED) {
        LogPrintf("CPrivateSendClient::CheckPoolStateUpdate -- entry is rejected by Goldminenode\n");
        UnlockCoins();
        keyHolderStorage.ReturnAll();
        SetNull();
        SetState(POOL_STATE_ERROR);
...
//
// After we receive the finalized transaction from the Goldminenode, we must
...
{
    if(fGoldminenodeMode || pnode == NULL) return false;
    ...
    if(finalMutableTransaction.GetHash() != finalTransactionNew.GetHash()) {
        LogPrintf("CPrivateSendClient::SignFinalTransaction -- WARNING! Goldminenode %s is not BIP69 compliant!\n", infoMixingGoldminenode.outpoint.ToStringShort());
        UnlockCoins();
        keyHolderStorage.ReturnAll();
        SetNull();
        return false;
    ...
    LogPrintf("CPrivateSendClient::SignFinalTransaction -- pushing sigs to the goldminenode, finalMutableTransaction=%s", finalMutableTransaction.ToString());
...
{
    if(fGoldminenodeMode) return false; // no client-side mixing on goldminenodes
    ...
    if(mnodeman.size() == 0) {
        LogPrint("privatesend", "CPrivateSendClient::DoAutomaticDenominating -- No Goldminenodes detected\n");
    ...
    // Initial phase, find a Goldminenode
    ...
    // If we've used 90% of the Goldminenode list then drop the oldest first ~30%
    int nThreshold_high = nMnCountEnabled * 0.9;
    int nThreshold_low = nThreshold_high * 0.7;
    LogPrint("privatesend", "Checking vecGoldminenodesUsed: size: %d, threshold: %d\n", (int)vecGoldminenodesUsed.size(), nThreshold_high);

    if((int)vecGoldminenodesUsed.size() > nThreshold_high) {
        vecGoldminenodesUsed.erase(vecGoldminenodesUsed.begin(), vecGoldminenodesUsed.begin() + vecGoldminenodesUsed.size() - nThreshold_low);
        LogPrint("privatesend", "  vecGoldminenodesUsed: new size: %d, threshold: %d\n", (int)vecGoldminenodesUsed.size(), nThreshold_high);
    }
    ...
    strAutoDenomResult = _("No compatible Goldminenode found.");
    return false;
}
...
bool CPrivateSendClient::JoinExistingQueue(CAmount nBalanceNeedsAnonymized, CConnman& connman)
{
    ...
    for (auto& dsq : vecDarksendQueue) {
        ...
        if(!mnodeman.GetGoldminenodeInfo(dsq.goldminenodeOutpoint, infoMn)) {
            LogPrintf("CPrivateSendClient::JoinExistingQueue -- dsq goldminenode is not in goldminenode list, goldminenode=%s\n", dsq.goldminenodeOutpoint.ToStringShort());
            continue;
        }
        ...
        if (mnpayments.IsScheduled(infoMn, 0)) {
            LogPrintf("CPrivateSendClient::JoinExistingQueue -- skipping winner, goldminenode=%s\n", infoMn.outpoint.ToStringShort());
            continue;
        }
        ...
        vecGoldminenodesUsed.push_back(dsq.goldminenodeOutpoint);

        if (connman.IsGoldminenodeOrDisconnectRequested(infoMn.addr)) {
            LogPrintf("CPrivateSendClient::JoinExistingQueue -- skipping goldminenode connection, addr=%s\n", infoMn.addr.ToString());
            continue;
        }

        nSessionDenom = dsq.nDenom;
        nSessionInputCount = fNewAlgo ? dsq.nInputCount : 0;
        infoMixingGoldminenode = infoMn;
        pendingDsaRequest = CPendingDsaRequest(infoMn.addr, CDarksendAccept(nSessionDenom, nSessionInputCount, txMyCollateral));
        connman.AddPendingGoldminenode(infoMn.addr);
...
bool CPrivateSendClient::StartNewQueue(CAmount nValueMin, CAmount nBalanceNeedsAnonymized, CConnman& connman)
{
    ...
    while(nTries < 10) {
        goldminenode_info_t infoMn = mnodeman.FindRandomNotInVec(vecGoldminenodesUsed, MIN_PRIVATESEND_PEER_PROTO_VERSION);

        if(!infoMn.fInfoValid) {
            LogPrintf("CPrivateSendClient::StartNewQueue -- Can't find random goldminenode!\n");
        ...
        // skip next mn payments winners
        if (mnpayments.IsScheduled(infoMn, 0)) {
            LogPrintf("CPrivateSendClient::StartNewQueue -- skipping winner, goldminenode=%s\n", infoMn.outpoint.ToStringShort());
            nTries++;
            continue;
        }

        vecGoldminenodesUsed.push_back(infoMn.outpoint);

        if(infoMn.nLastDsq != 0 && infoMn.nLastDsq + nMnCountEnabled/5 > mnodeman.nDsqCount) {
            LogPrintf("CPrivateSendClient::StartNewQueue -- Too early to mix on this goldminenode!"
        ...
        if (connman.IsGoldminenodeOrDisconnectRequested(infoMn.addr)) {
            LogPrintf("CPrivateSendClient::StartNewQueue -- skipping goldminenode connection, addr=%s\n", infoMn.addr.ToString());
            nTries++;
            continue;
        }

        LogPrintf("CPrivateSendClient::StartNewQueue -- attempt %d connection to Goldminenode %s\n", nTries, infoMn.addr.ToString());
...
void CPrivateSendClient::RelayIn(const CDarkSendEntry& entry, CConnman& connman)
{
    ...
    connman.ForNode(infoMixingGoldminenode.addr, [&entry, &connman](CNode* pnode) {
...
void ThreadCheckPrivateSendClient(CConnman& connman)
{
    ...
    if(fGoldminenodeMode) return; // no client-side mixing on goldminenodes
