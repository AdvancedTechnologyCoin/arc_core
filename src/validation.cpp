...
{
    if(sporkManager.IsSporkActive(SPORK_24_UPGRADE))
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    for (const auto& tx : block.vtx)
    {
        ...
        LogPrintf("Check transactions tx hash %s\n",tx->GetHash().ToString());
    }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Check proof of work
    if(Params().NetworkIDString() == CBaseChainParams::MAIN && nHeight <= 97961){
...
static bool AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock)
{
    ...
    //ARC BLOCKCHAIN TX CHECK
    if(sporkManager.IsSporkActive(SPORK_24_UPGRADE) && pindex->nHeight >= 1016955 && pblock->vtx[0]->vout.size() != 3) 
    {
        LogPrintf("Error: reject block, because is have only %d transaction of 3\n",pblock->vtx.size());
        return false;
    }
    //END ARC CHECK
