...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized arc balance of the wallet\n"
...
UniValue mnsync(const JSONRPCRequest& request)
{
    ...
    if(strMode == "status") {
        ...
        objStatus.push_back(Pair("AssetID", goldminenodeSync.GetAssetID()));
        objStatus.push_back(Pair("AssetName", goldminenodeSync.GetAssetName()));
        objStatus.push_back(Pair("AssetStartTime", goldminenodeSync.GetAssetStartTime()));
        objStatus.push_back(Pair("Attempt", goldminenodeSync.GetAttempt()));
        objStatus.push_back(Pair("IsBlockchainSynced", goldminenodeSync.IsBlockchainSynced()));
        objStatus.push_back(Pair("IsGoldminenodeListSynced", goldminenodeSync.IsGoldminenodeListSynced()));
        objStatus.push_back(Pair("IsWinnersListSynced", goldminenodeSync.IsWinnersListSynced()));
        objStatus.push_back(Pair("IsSynced", goldminenodeSync.IsSynced()));
        objStatus.push_back(Pair("IsFailed", goldminenodeSync.IsFailed()));
        return objStatus;
    }

    if(strMode == "next")
    {
        goldminenodeSync.SwitchToNextAsset(*g_connman);
        return "sync updated to " + goldminenodeSync.GetAssetName();
    }

    if(strMode == "reset")
    {
        goldminenodeSync.Reset();
        goldminenodeSync.SwitchToNextAsset(*g_connman);
        return "success";
    }
    return "failure";
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"     (string, required) The arc address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The arc address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Arc address and we have full public key:
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The arc address to use for the signature.\n"
