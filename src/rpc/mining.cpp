...
UniValue generateBlocks(boost::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript)
{
    ...
    while (nHeight < nHeightEnd)
    {
        ...
        LogPrintf("generateBlocks voutSuperblock size %d\n",pblock->voutSuperblock.size());
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. nblocks      (numeric, required) How many blocks are generated immediately.\n"
            "2. address      (string, required) The address to send the newly generated Arc to.\n"
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "  \"goldminenode\" : {                  (json object) required goldminenode payee that must be included in the next block\n"
            "      \"payee\" : \"xxxx\",             (string) payee address\n"
            "      \"script\" : \"xxxx\",            (string) payee scriptPubKey\n"
            "      \"amount\": n                   (numeric) required amount to pay\n"
            "  },\n"
            "  \"goldminenode_payments_started\" :  true|false, (boolean) true, if goldminenode payments started\n"
            "  \"goldminenode_payments_enforced\" : true|false, (boolean) true, if goldminenode payments are enforced\n"
    ...
    if (Params().MiningRequiresPeers()) {
        if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
            throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Arc Core is not connected!");

        if (IsInitialBlockDownload())
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Arc Core is downloading blocks...");
    }

    // when enforcement is on we need information about a goldminenode payee or otherwise our block is going to be orphaned by the network
    CScript payee;
    if (sporkManager.IsSporkActive(SPORK_8_GOLDMINENODE_PAYMENT_ENFORCEMENT)
        && !goldminenodeSync.IsWinnersListSynced()
        && !mnpayments.GetBlockPayee(chainActive.Height() + 1, payee))
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Arc Core is downloading goldminenode winners...");
    ...
    UniValue goldminenodeObj(UniValue::VOBJ);
    if(pblock->txoutGoldminenode != CTxOut()) {
        CTxDestination address1;
        ExtractDestination(pblock->txoutGoldminenode.scriptPubKey, address1);
        CBitcoinAddress address2(address1);
        goldminenodeObj.push_back(Pair("payee", address2.ToString().c_str()));
        goldminenodeObj.push_back(Pair("script", HexStr(pblock->txoutGoldminenode.scriptPubKey)));
        goldminenodeObj.push_back(Pair("amount", pblock->txoutGoldminenode.nValue));
    }
    result.push_back(Pair("goldminenode", goldminenodeObj));
    result.push_back(Pair("goldminenode_payments_started", pindexPrev->nHeight + 1 > consensusParams.nGoldminenodePaymentsStartBlock));
	...
	bool bWk1 = sporkManager.IsSporkActive(SPORK_18_EVOLUTION_PAYMENTS);
