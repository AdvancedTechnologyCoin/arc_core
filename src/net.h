...
public:
    ...
    // fConnectToGoldminenode should be 'true' only if you want this node to allow to connect to itself
    // and/or you want it to be disconnected on CGoldminenodeMan::ProcessGoldminenodeConnections()
    // Unfortunately, can't make this method private like in Bitcoin,
    // because it's used in many Arc-specific places (goldminenode, privatesend).
    CNode* ConnectNode(CAddress addrConnect, const char *pszDest = NULL, bool fConnectToGoldminenode = false);
