...
void CMasternodeSync::GetNextAsset()
{
    switch(RequestedMasternodeAssets)
    {
        ...
        case(MASTERNODE_SYNC_BUDGET):
            LogPrintf("CGoldnineNodeSync::GetNextAsset - Sync has finished\n");
...
std::string CMasternodeSync::GetSyncStatus()
{
    switch (masternodeSync.RequestedMasternodeAssets) {
        ...
        case MASTERNODE_SYNC_LIST: return _("Synchronizing goldmines...");
        case MASTERNODE_SYNC_MNW: return _("Synchronizing goldmine winners...");
        case MASTERNODE_SYNC_BUDGET: return _("Synchronizing evolutions...");
...
void CMasternodeSync::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    if (strCommand == "ssc") { //Sync status count
        ...
        LogPrintf("CGoldnineNodeSync:ProcessMessage - ssc - got inventory count %d %d\n", nItemID, nCount);
    }
}
...
void CMasternodeSync::Process()
{
    ...
    if(fDebug) LogPrintf("CGoldnineNodeSync::Process() - tick %d RequestedGoldmineAssets %d\n", tick, RequestedMasternodeAssets);
    ...
    BOOST_FOREACH(CNode* pnode, vNodes)
    {
        ...
        if (pnode->nVersion >= masternodePayments.GetMinMasternodePaymentsProto()) {
            ...
            if(RequestedMasternodeAssets == MASTERNODE_SYNC_LIST) {
                if(fDebug) LogPrintf("CGoldnineNodeSync::Process() - lastMasternodeList %lld (GetTime() - MASTERNODE_SYNC_TIMEOUT) %lld\n", lastMasternodeList, GetTime() - MASTERNODE_SYNC_TIMEOUT);
                ...
                (RequestedMasternodeAttempt >= MASTERNODE_SYNC_THRESHOLD*3 || GetTime() - nAssetSyncStarted > MASTERNODE_SYNC_TIMEOUT*5)) {
                    if(IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)) {
                        LogPrintf("CGoldnineNodeSync::Process - ERROR - Sync has failed, will retry later\n");
        ...
        if (pnode->nVersion >= MIN_BUDGET_PEER_PROTO_VERSION) {
            ...
            if(RequestedMasternodeAssets == MASTERNODE_SYNC_BUDGET){
                ...
                if(lastBudgetItem > 0 && lastBudgetItem < GetTime() - MASTERNODE_SYNC_TIMEOUT*2 && RequestedMasternodeAttempt >= MASTERNODE_SYNC_THRESHOLD){ //hasn't received a new item in the last five seconds, so we'll move to the
                    //LogPrintf("CGoldnineNodeSync::Process - HasNextFinalizedBudget %d nCountFailures %d IsBudgetPropEmpty %d\n", budget.HasNextFinalizedBudget(), nCountFailures, IsBudgetPropEmpty());
