...
void CSpysendPool::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if(strCommand == NetMsgType::DSACCEPT) {
        ...
        CGoldminenode* pmn = mnodeman.Find(activeGoldminenode.vin);
        if(pmn == NULL) {
        ...
        if(!dsq.CheckSignature(pmn->pubKeyGoldminenode)) {
        ...
        if(dsq.fReady) {
            ...
            if((CNetAddr)pSubmittedToGoldminenode->addr != (CNetAddr)pmn->addr) {
                LogPrintf("DSQUEUE -- message doesn't match current Goldminenode: pSubmittedToGoldminenode=%s, addr=%s\n", pSubmittedToGoldminenode->addr.ToString(), pmn->addr.ToString());
                return;
            }
        ...
        if(!pSubmittedToGoldminenode) return;
        if((CNetAddr)pSubmittedToGoldminenode->addr != (CNetAddr)pfrom->addr) {
            //LogPrintf("DSSTATUSUPDATE -- message doesn't match current Goldminenode: pSubmittedToGoldminenode %s addr %s\n", pSubmittedToGoldminenode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if(!pSubmittedToGoldminenode) return;
        if((CNetAddr)pSubmittedToGoldminenode->addr != (CNetAddr)pfrom->addr) {
            //LogPrintf("DSFINALTX -- message doesn't match current Goldminenode: pSubmittedToGoldminenode %s addr %s\n", pSubmittedToGoldminenode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if(!pSubmittedToGoldminenode) return;
        if((CNetAddr)pSubmittedToGoldminenode->addr != (CNetAddr)pfrom->addr) {
            LogPrint("privatesend", "DSCOMPLETE -- message doesn't match current Goldminenode: pSubmittedToGoldminenode=%s  addr=%s\n", pSubmittedToGoldminenode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
...
std::string CSpysendPool::GetStatus()
{
    ...
    if((pCurrentBlockIndex && pCurrentBlockIndex->nHeight - nCachedLastSuccessBlock < nMinBlockSpacing) || !goldminenodeSync.IsBlockchainSynced())
    ...
    switch(nState) {
        ...
        case POOL_STATE_ACCEPTING_ENTRIES:
            if(nEntriesCount == 0) {
                ...
                if(     nStatusMessageProgress % 70 <= 40) return strprintf(_("Submitted following entries to goldminenode: %u / %d"), nEntriesCount, GetMaxPoolTransactions());
                else if(nStatusMessageProgress % 70 <= 50) strSuffix = ".";
                else if(nStatusMessageProgress % 70 <= 60) strSuffix = "..";
                else if(nStatusMessageProgress % 70 <= 70) strSuffix = "...";
                return strprintf(_("Submitted to goldminenode, waiting for more entries ( %u / %d ) %s"), nEntriesCount, GetMaxPoolTransactions(), strSuffix);
            }
...
//
// How does this work? Messages to Goldminenodes come in via NetMsgType::DSVIN, these require a valid collateral
...
/*
    ...
    allow endless transaction that would bloat Arc and make it unusable. To
...
{
    if(fGoldmineNode) {
        LogPrintf("CSpysendPool::SendDenominate -- PrivateSend from a Goldminenode is not supported currently.\n");
        return false;
    }
...
bool CSpysendPool::CheckPoolStateUpdate(PoolState nStateNew, int nEntriesCountNew, PoolStatusUpdate nStatusUpdate, PoolMessage nMessageID, int nSessionIDNew)
{
    ...
    strAutoDenomResult = _("Goldminenode:") + " " + GetMessageByID(nMessageID);
...
{
    if(!fEnablePrivateSend || fGoldmineNode || !pCurrentBlockIndex) return false;
...
std::string CSpysendPool::GetMessageByID(PoolMessage nMessageID)
{
    switch (nMessageID) {
        ...
        case ERR_MN_LIST:               return _("Not in the Goldminenode list.");
        case ERR_MODE:                  return _("Incompatible mode.");
        case ERR_NON_STANDARD_PUBKEY:   return _("Non-standard public key detected.");
        case ERR_NOT_A_MN:              return _("This is not a Goldminenode.");
        case ERR_QUEUE_FULL:            return _("Goldminenode queue is full.");
...
bool CSpysendQueue::Sign()
{
    ...
    if(!spySendSigner.SignMessage(strMessage, vchSig, activeGoldminenode.keyGoldminenode)) {
...
bool CSpysendQueue::CheckSignature(const CPubKey& pubKeyGoldminenode)
{
    ...
    if(!spySendSigner.VerifyMessage(pubKeyGoldminenode, vchSig, strMessage, strError)) {
