...
void BitcoinGUI::createActions()
{
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and arc: URIs)"));
...
#ifdef ENABLE_WALLET
    QSettings settings;
    if (!fLiteMode && settings.value("fShowGoldminenodesTab").toBool()) {
    ...
    openAction->setStatusTip(tr("Open a arc: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Arc command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::createToolBars()
{
    ...
    if(walletFrame)
    {
        ...
        QSettings settings;
        if (!fLiteMode && settings.value("fShowGoldminenodesTab").toBool() && goldminenodeAction)
        {
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Arc network", "", count));
    } else {
