    ...
    platformStyle(platformStyle)
{
    ...
    QString windowTitle = tr("Arctic Core") + " - ";
#ifdef ENABLE_WALLET
...
void BitcoinGUI::createActions()
{
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and arcticcoin: URIs)"));
...
#ifdef ENABLE_WALLET
    QSettings settings;
    if (settings.value("fShowGoldminenodesTab").toBool()) {
        goldminenodeAction = new QAction(QIcon(":/icons/" + theme + "/goldminenodes"), tr("&Goldminenodes"), this);
        goldminenodeAction->setStatusTip(tr("Browse goldminenodes"));
        goldminenodeAction->setToolTip(goldminenodeAction->statusTip());
        goldminenodeAction->setCheckable(true);
#ifdef Q_OS_MAC
        goldminenodeAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_5));
#else
        goldminenodeAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_5));
#endif
        tabGroup->addAction(goldminenodeAction);
        connect(goldminenodeAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
        connect(goldminenodeAction, SIGNAL(triggered()), this, SLOT(gotoGoldminenodePage()));
    }
    ...
    aboutAction = new QAction(QIcon(":/icons/" + theme + "/about"), tr("&About Arctic Core"), this);
    aboutAction->setStatusTip(tr("Show information about Arctic Core"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for Arctic Core"));
    ...
    openMNConfEditorAction = new QAction(QIcon(":/icons/" + theme + "/edit"), tr("Open &Goldminenode Configuration File"), this);
    openMNConfEditorAction->setStatusTip(tr("Open Goldminenode configuration file"));    
    ...
    openAction->setStatusTip(tr("Open a arcticcoin: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the Arctic Core help message to get a list with possible Arctic Core command-line options"));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        if (settings.value("fShowGoldminenodesTab").toBool())
        {
            toolbar->addAction(goldminenodeAction);
        }
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    QSettings settings;
    if (settings.value("fShowGoldminenodesTab").toBool() && goldminenodeAction) {
        goldminenodeAction->setEnabled(enabled);
    }
...
void BitcoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    ...
    QString toolTip = tr("Arctic Core client") + " " + networkStyle->getTitleAddText();
...
void BitcoinGUI::gotoGoldminenodePage()
{
    QSettings settings;
    if (settings.value("fShowGoldminenodesTab").toBool()) {
        goldminenodeAction->setChecked(true);
        if (walletFrame) walletFrame->gotoGoldminenodePage();
    }
}
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Arctic network", "", count));
}
...
void BitcoinGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    ...
    if(goldminenodeSync.IsBlockchainSynced())
    {
        ...
        strSyncStatus = QString(goldminenodeSync.GetSyncStatus().c_str());
...
{
    QString strTitle = tr("Arctic Core"); // default title
