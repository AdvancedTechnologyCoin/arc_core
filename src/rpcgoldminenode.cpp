...
UniValue getpoolinfo(const UniValue& params, bool fHelp)
{
    ...
    if (darkSendPool.pSubmittedToGoldminenode) {
        obj.push_back(Pair("outpoint",      darkSendPool.pSubmittedToGoldminenode->vin.prevout.ToStringShort()));
        obj.push_back(Pair("addr",          darkSendPool.pSubmittedToGoldminenode->addr.ToString()));
    }
...
UniValue goldminenode(const UniValue& params, bool fHelp)
{
        ...
        (strCommand != "start" && strCommand != "start-alias" && strCommand != "start-all" && strCommand != "start-missing" &&
         ...
         strCommand != "connect" && strCommand != "outputs" && strCommand != "status"))
            throw std::runtime_error(
                ...
                "  start        - Start local Hot goldminenode configured in arcticcoin.conf\n"
    ...
    if (strCommand == "count")
    {
        ...
        int nCount;
        mnodeman.GetNextGoldminenodeInQueueForPayment(true, nCount);
    ...
    if (strCommand == "current" || strCommand == "winner")
    {
        int nCount;
        int nHeight;
        CGoldminenode* winner = NULL;
        {
            LOCK(cs_main);
            nHeight = chainActive.Height() + (strCommand == "current" ? 1 : 10);
        }
        mnodeman.UpdateLastPaid();
        winner = mnodeman.GetNextGoldminenodeInQueueForPayment(nHeight, true, nCount);
        ...
        obj.push_back(Pair("lastseen",      (winner->lastPing == CGoldminenodePing()) ? winner->sigTime :
                                                    winner->lastPing.sigTime));
        obj.push_back(Pair("activeseconds", (winner->lastPing == CGoldminenodePing()) ? 0 :
    ...
    if (strCommand == "debug")
    {
        if(activeGoldminenode.nState != ACTIVE_GOLDMINENODE_INITIAL || !goldminenodeSync.IsBlockchainSynced())
            return activeGoldminenode.GetStatus();

        CTxIn vin;
        CPubKey pubkey;
        CKey key;

        if(!pwalletMain || !pwalletMain->GetGoldminenodeVinAndKeys(vin, pubkey, key))
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Missing goldminenode input, please look at the documentation for instructions on goldminenode creation");
    ...
    if (strCommand == "start")
    {
        ...
        if(activeGoldminenode.nState != ACTIVE_GOLDMINENODE_STARTED){
            activeGoldminenode.nState = ACTIVE_GOLDMINENODE_INITIAL; // TODO: consider better way
    ...
    if (strCommand == "start-all" || strCommand == "start-missing" || strCommand == "start-disabled")
    {
        ...
        BOOST_FOREACH(CGoldminenodeConfig::CGoldminenodeEntry mne, goldminenodeConfig.getEntries()) {
            ...
            CGoldminenode *pmn = mnodeman.Find(vin);
            CGoldminenodeBroadcast mnb;
    ...
    if (strCommand == "status")
    {
        ...
        mnObj.push_back(Pair("vin", activeGoldminenode.vin.ToString()));
        mnObj.push_back(Pair("service", activeGoldminenode.service.ToString()));

        CGoldminenode mn;
        if(mnodeman.Get(activeGoldminenode.vin, mn)) {
...
UniValue goldminenodelist(const UniValue& params, bool fHelp)
{
                ...
                strMode != "protocol" && strMode != "payee" && strMode != "rank" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print Arctic address associated with a goldminenode (can be additionally filtered,\n"
                "                   partial match)\n"
    ...
    UniValue obj(UniValue::VOBJ);
    if (strMode == "rank") {
        std::vector<std::pair<int, CGoldminenode> > vGoldminenodeRanks = mnodeman.GetGoldminenodeRanks();
...
UniValue goldminenodebroadcast(const UniValue& params, bool fHelp)
{
    ...
    if (strCommand == "relay")
    {
        ...
        BOOST_FOREACH(CGoldminenodeBroadcast& mnb, vecMnb) {
            ...
            if (mnb.CheckSignature(nDos)) {
                if (fSafe) {
                    fResult = mnodeman.CheckMnbAndUpdateGoldminenodeList(NULL, mnb, nDos);
                } else {
                    mnodeman.UpdateGoldminenodeList(mnb);
                    mnb.Relay();
                    fResult = true;
