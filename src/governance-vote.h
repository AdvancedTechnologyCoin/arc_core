...
// INTENTION OF GOLDMINENODES REGARDING ITEM
...
//
// CGovernanceVote - Allow a goldminenode node to vote and broadcast throughout the network
//
...
public:
    CGovernanceVote();
    CGovernanceVote(CTxIn vinGoldminenodeIn, uint256 nParentHashIn, vote_signal_enum_t eVoteSignalIn, vote_outcome_enum_t eVoteOutcomeIn);
    ...
    CTxIn& GetVinGoldminenode() { return vinGoldminenode; }

    const CTxIn& GetVinGoldminenode() const { return vinGoldminenode; }
    ...
    *   GetTypeHash()
    *
    *   GET HASH WITH DETERMINISTIC VALUE OF GOLDMINENODE-VIN/PARENT-HASH/VOTE-SIGNAL
    *
    *   This hash collides with previous goldminenode votes when they update their votes on governance objects.
    ...
    *   --
    *
    *   We do not include an outcome, because that can change when a goldminenode updates their vote from yes to no
