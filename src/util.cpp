...
const char * const BITCOIN_CONF_FILENAME = "arcticcoin.conf";
const char * const BITCOIN_PID_FILENAME = "arcticcoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            if(ptrCategory->count(string("arcticcoin"))) {
                ...
                ptrCategory->insert(string("goldminenode"));
...
#else
    const char* pszModule = "arcticcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ArcticCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ArcticCore";
#else
    // Unix
    return pathRet / ".arcticcore";
#endif
#endif
}
...
boost::filesystem::path GetGoldminenodeConfigFile()
{
    boost::filesystem::path pathConfigFile(GetArg("-mnconf", "goldminenode.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty arcticcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override arcticcoin.conf
