...
CGovernanceVote::CGovernanceVote(CTxIn vinGoldminenodeIn, uint256 nParentHashIn, vote_signal_enum_t eVoteSignalIn, vote_outcome_enum_t eVoteOutcomeIn)
    : fValid(true),
      fSynced(false),
...
bool CGovernanceVote::Sign(CKey& keyGoldminenode, CPubKey& pubKeyGoldminenode)
{
    // Choose coins to use
    ...
    std::string strError;
    std::string strMessage = vinGoldminenode.prevout.ToStringShort() + "|" + nParentHash.ToString() + "|" +
...
bool CGovernanceVote::IsValid(bool fSignatureCheck) const
{
    ...
    if(!infoMn.fInfoValid) {
        LogPrint("gobject", "CGovernanceVote::IsValid -- Unknown Goldminenode - %s\n", vinGoldminenode.prevout.ToStringShort());
        return false;
    }
    ...
    if(!darkSendSigner.VerifyMessage(infoMn.pubKeyGoldminenode, vchSig, strMessage, strError)) {
...
{
    bool fResult = ((vote1.vinGoldminenode == vote2.vinGoldminenode) &&
...
bool operator<(const CGovernanceVote& vote1, const CGovernanceVote& vote2)
{
    ...
    fResult = (vote1.vinGoldminenode == vote2.vinGoldminenode);
