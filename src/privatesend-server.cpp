...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(strCommand == NetMsgType::DSACCEPT) {
        ...
        if(!mnodeman.GetGoldminenodeInfo(activeGoldminenode.outpoint, mnInfo)) {
        ...
        goldminenode_info_t mnInfo;
        if(!mnodeman.GetGoldminenodeInfo(dsq.goldminenodeOutpoint, mnInfo)) return;

        if(!dsq.CheckSignature(mnInfo.pubKeyGoldminenode)) {
        ...
        if(!dsq.fReady) {
            ...
            mnodeman.AllowMixing(dsq.goldminenodeOutpoint);
...
{
    if(!fGoldminenodeMode) return; // check and relay final tx only on goldminenode
    ...
    if(!CPrivateSend::GetDSTX(hashTx)) {
        CDarksendBroadcastTx dstxNew(finalTransaction, activeGoldminenode.outpoint, GetAdjustedTime());
        dstxNew.Sign();
...
void CPrivateSendServer::CheckForCompleteQueue(CConnman& connman)
{
    ...
    if(nState == POOL_STATE_QUEUE && IsSessionReady()) {
        ...
        CDarksendQueue dsq(nSessionDenom, nSessionInputCount, activeGoldminenode.outpoint, GetAdjustedTime(), true);
...
{
    if(!fGoldminenodeMode || nSessionID != 0) return false;
    ...
    if(!fUnitTest) {
        ...
        CDarksendQueue dsq(dsa.nDenom, dsa.nInputCount, activeGoldminenode.outpoint, GetAdjustedTime(), false);
...
{
    if(!fGoldminenodeMode || nSessionID == 0 || IsSessionReady()) return false;
...
void CPrivateSendServer::SetState(PoolState nStateNew)
{
    ...
    if(nStateNew == POOL_STATE_ERROR || nStateNew == POOL_STATE_SUCCESS) {
        LogPrint("privatesend", "CPrivateSendServer::SetState -- Can't set state to ERROR or SUCCESS as a Goldminenode. \n");
        return;
    }
...
void ThreadCheckPrivateSendServer(CConnman& connman)
{
    ...
    if(!fGoldminenodeMode) return; // only run on goldminenodes
