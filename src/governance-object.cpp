...
: cs(),
  ...
  vinGoldminenode(other.vinGoldminenode),
  vchSig(other.vchSig),
...
{
    int nMNIndex = governance.GetGoldminenodeIndex(vote.GetVinGoldminenode());
    if(nMNIndex < 0) {
        std::ostringstream ostr;
        ostr << "CGovernanceObject::ProcessVote -- Goldminenode index not found\n";
        exception = CGovernanceException(ostr.str(), GOVERNANCE_EXCEPTION_WARNING);
        if(mapOrphanVotes.Insert(vote.GetVinGoldminenode(), vote_time_pair_t(vote, GetAdjustedTime() + GOVERNANCE_ORPHAN_EXPIRATION_TIME))) {
            if(pfrom) {
                mnodeman.AskForMN(pfrom, vote.GetVinGoldminenode());
            }
    ...
    if(governance.AreRateChecksEnabled()) {
        ...
        if(nTimeDelta < GOVERNANCE_UPDATE_MIN) {
            std::ostringstream ostr;
            ostr << "CGovernanceObject::ProcessVote -- Goldminenode voting too often "
...
void CGovernanceObject::RebuildVoteMap()
{
    ...
    for(vote_m_it it = mapCurrentMNVotes.begin(); it != mapCurrentMNVotes.end(); ++it) {
        ...
        if(mnodeman.GetGoldminenodeVinForIndexOld(it->first, vinGoldminenode)) {
            int nNewIndex = mnodeman.GetGoldminenodeIndex(vinGoldminenode);
            if((nNewIndex >= 0)) {
...
void CGovernanceObject::ClearGoldminenodeVotes()
{
    ...
    while(it != mapCurrentMNVotes.end()) {
        ...
        CTxIn vinGoldminenode;
        bool fRemove = true;
        if(mnodeman.Get(it->first, vinGoldminenode, fIndexRebuilt)) {
            if(mnodeman.Has(vinGoldminenode)) {
                fRemove = false;
            }
            else {
                fileVotes.RemoveVotesFromGoldminenode(vinGoldminenode);
            }
        }
...
std::string CGovernanceObject::GetSignatureMessage() const
{
    ...
    std::string strMessage = nHashParent.ToString() + "|" +
        ...
        strData + "|" +
        vinGoldminenode.prevout.ToStringShort() + "|" +
        nCollateralHash.ToString();

    return strMessage;
}

void CGovernanceObject::SetGoldminenodeInfo(const CTxIn& vin)
{
    vinGoldminenode = vin;
}

bool CGovernanceObject::Sign(CKey& keyGoldminenode, CPubKey& pubKeyGoldminenode)
{
    std::string strError;
    ...
    LogPrint("gobject", "CGovernanceObject::Sign -- pubkey id = %s, vin = %s\n",
             pubKeyGoldminenode.GetID().ToString(), vinGoldminenode.prevout.ToStringShort());


    return true;
}

bool CGovernanceObject::CheckSignature(CPubKey& pubKeyGoldminenode)
{
    std::string strError;
...
{
    bool fMissingGoldminenode = false;

    return IsValidLocally(strError, fMissingGoldminenode, fCheckCollateral);
}

bool CGovernanceObject::IsValidLocally(std::string& strError, bool& fMissingGoldminenode, bool fCheckCollateral)
{
    fMissingGoldminenode = false;
    ...
    // IF ABSOLUTE NO COUNT (NO-YES VALID VOTES) IS MORE THAN 10% OF THE NETWORK GOLDMINENODES, OBJ IS INVALID

    // CHECK COLLATERAL IF REQUIRED (HIGH CPU USAGE)

    if(fCheckCollateral) { 
        if((nObjectType == GOVERNANCE_OBJECT_TRIGGER) || (nObjectType == GOVERNANCE_OBJECT_WATCHDOG)) {
            std::string strOutpoint = vinGoldminenode.prevout.ToStringShort();
            goldminenode_info_t infoMn = mnodeman.GetGoldminenodeInfo(vinGoldminenode);
            if(!infoMn.fInfoValid) {
                fMissingGoldminenode = true;
                strError = "Goldminenode not found: " + strOutpoint;
                return false;
            }

            // Check that we have a valid MN signature
            if(!CheckSignature(infoMn.pubKeyGoldminenode)) {
                strError = "Invalid goldminenode signature for: " + strOutpoint + ", pubkey id = " + infoMn.pubKeyGoldminenode.GetID().ToString();
                return false;
            }
...
{
    int nMNIndex = governance.GetGoldminenodeIndex(mnCollateralOutpoint);
    vote_m_it it = mapCurrentMNVotes.find(nMNIndex);
    if (it == mapCurrentMNVotes.end()) {
        return false;
    }
...
void CGovernanceObject::CheckOrphanVotes()
{
    ...
    while(it != listVotes.end()) {
        ...
        else if(!mnodeman.Has(vote.GetVinGoldminenode())) {
            ++it;
            continue;
        }
        CGovernanceException exception;
