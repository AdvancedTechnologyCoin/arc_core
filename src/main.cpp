...
bool ProcessNewBlock(CValidationState& state, const CChainParams& chainparams, const CNode* pfrom, const CBlock* pblock, bool fForceProcessing, CDiskBlockPos* dbp)
{
    ...
    goldminenodeSync.IsBlockchainSynced(true);
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        Arctic Related Inventory Messages
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        LogPrint("net", "ProcessGetData -- inv = %s\n", inv.ToString());
        {
            ...
            else if (inv.IsKnownType())
            {
                ...
                if (!pushed && inv.type == MSG_GOLDMINENODE_PAYMENT_VOTE) {
                    if(mnpayments.HasVerifiedPaymentVote(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnpayments.mapGoldminenodePaymentVotes[inv.hash];
                        pfrom->PushMessage(NetMsgType::GOLDMINENODEPAYMENTVOTE, ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_GOLDMINENODE_PAYMENT_BLOCK) {
                    BlockMap::iterator mi = mapBlockIndex.find(inv.hash);
                        ...
                        BOOST_FOREACH(CGoldminenodePayee& payee, mnpayments.mapGoldminenodeBlocks[mi->second->nHeight].vecPayees) {
                            ...
                            BOOST_FOREACH(uint256& hash, vecVoteHashes) {
                                if(mnpayments.HasVerifiedPaymentVote(hash)) {
                                    CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                                    ss.reserve(1000);
                                    ss << mnpayments.mapGoldminenodePaymentVotes[hash];
                                    pfrom->PushMessage(NetMsgType::GOLDMINENODEPAYMENTVOTE, ss);
                                }
                            }
                        }
                ...
                if (!pushed && inv.type == MSG_GOLDMINENODE_ANNOUNCE) {
                ...
                if (!pushed && inv.type == MSG_GOLDMINENODE_PING) {
                    if(mnodeman.mapSeenGoldminenodePing.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenGoldminenodePing[inv.hash];
                ...
                if (!pushed && inv.type == MSG_GOLDMINENODE_VERIFY) {
                    if(mnodeman.mapSeenGoldminenodeVerification.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenGoldminenodeVerification[inv.hash];
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    else if (strCommand == NetMsgType::TX || strCommand == NetMsgType::DSTX || strCommand == NetMsgType::TXLOCKREQUEST)
    {
        ...
        if (strCommand == NetMsgType::TXLOCKREQUEST) {
            ...
            CGoldminenode* pmn = mnodeman.Find(dstx.vin);
            if(pmn == NULL) {
            ...
            if(!dstx.CheckSignature(pmn->pubKeyGoldminenode)) {
