...
void CGovernanceManager::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
...

    {
        ...
        if(!goldminenodeSync.IsGoldminenodeListSynced()) {
            LogPrint("gobject", "MNGOVERNANCEOBJECT -- goldminenode list not synced\n");
            return;
        }
        ...
        if(!GoldminenodeRateCheck(govobj, true, false, fRateCheckBypassed)) {
            LogPrintf("MNGOVERNANCEOBJECT -- goldminenode rate check failed - %s - (current block height %d) \n", strHash, nCachedBlockHeight);
            return;
        }
        ...
        bool fGoldminenodeMissing = false;
        bool fIsValid = govobj.IsValidLocally(strError, fGoldminenodeMissing, true);

        if(fGoldminenodeMissing) {
            mapGoldminenodeOrphanObjects.insert(std::make_pair(nHash, object_time_pair_t(govobj, GetAdjustedTime() + GOVERNANCE_ORPHAN_EXPIRATION_TIME)));
            LogPrintf("MNGOVERNANCEOBJECT -- Missing goldminenode for: %s, strError = %s\n", strHash, strError);
        ...
        if(fRateCheckBypassed) {
            if(!GoldminenodeRateCheck(govobj, true, true, fRateCheckBypassed)) {
                LogPrintf("MNGOVERNANCEOBJECT -- goldminenode rate check failed (after signature verification) - %s - (current block height %d) \n", strHash, nCachedBlockHeight);
                return;
            }
        }
        ...
        goldminenodeSync.AddedGovernanceItem();
    ...
    {
        // Ignore such messages until goldminenode list is synced
        if(!goldminenodeSync.IsGoldminenodeListSynced()) {
            LogPrint("gobject", "MNGOVERNANCEOBJECTVOTE -- goldminenode list not synced\n");
            return;
        }
        ...
        if(ProcessVote(pfrom, vote, exception)) {
            ...
            if((exception.GetNodePenalty() != 0) && goldminenodeSync.IsSynced()) {
...
void CGovernanceManager::UpdateCachesAndClean()
{
    ...
    for(size_t i = 0; i < vecDirtyHashes.size(); ++i) {
        ...
        it->second.ClearGoldminenodeVotes();
...
std::vector<CGovernanceVote> CGovernanceManager::GetCurrentVotes(const uint256& nParentHash, const CTxIn& mnCollateralOutpointFilter)
{
    ...
    // Compile a list of Goldminenode collateral outpoints for which to get votes
    std::vector<CTxIn> vecMNTxIn;
    if (mnCollateralOutpointFilter == CTxIn()) {
        std::vector<CGoldminenode> mnlist = mnodeman.GetFullGoldminenodeVector();
        for (std::vector<CGoldminenode>::iterator it = mnlist.begin(); it != mnlist.end(); ++it)
        {
...
void CGovernanceManager::Sync(CNode* pfrom, const uint256& nProp, const CBloomFilter& filter)
{
    ...
    */
    ...
    if(fGoldmineNode && !goldminenodeSync.IsSynced()) return;
    ...
    pfrom->PushMessage(NetMsgType::SYNCSTATUSCOUNT, GOLDMINENODE_SYNC_GOVOBJ, nObjCount);
    pfrom->PushMessage(NetMsgType::SYNCSTATUSCOUNT, GOLDMINENODE_SYNC_GOVOBJ_VOTE, nVoteCount);
    LogPrintf("CGovernanceManager::Sync -- sent %d objects and %d votes to peer=%d\n", nObjCount, nVoteCount, pfrom->id);
}

bool CGovernanceManager::GoldminenodeRateCheck(const CGovernanceObject& govobj, bool fUpdateLast)
{
    bool fRateCheckBypassed = false;
    return GoldminenodeRateCheck(govobj, fUpdateLast, true, fRateCheckBypassed);
}

bool CGovernanceManager::GoldminenodeRateCheck(const CGovernanceObject& govobj, bool fUpdateLast, bool fForce, bool& fRateCheckBypassed)
{
    LOCK(cs);
    ...
    const CTxIn& vin = govobj.GetGoldminenodeVin();

    txout_m_it it  = mapLastGoldminenodeObject.find(vin.prevout);

    if(it == mapLastGoldminenodeObject.end()) {
        if(fUpdateLast) {
            it = mapLastGoldminenodeObject.insert(txout_m_t::value_type(vin.prevout, last_object_rec(true))).first;
            switch(nObjectType) {
            case GOVERNANCE_OBJECT_TRIGGER:
    ...
    if(nTimestamp < nNow - 2 * nSuperblockCycleSeconds) {
        LogPrintf("CGovernanceManager::GoldminenodeRateCheck -- object %s rejected due to too old timestamp, goldminenode vin = %s, timestamp = %d, current time = %d\n",
                 strHash, vin.prevout.ToStringShort(), nTimestamp, nNow);
        return false;
    }

    if(nTimestamp > nNow + 60*60) {
        LogPrintf("CGovernanceManager::GoldminenodeRateCheck -- object %s rejected due to too new (future) timestamp, goldminenode vin = %s, timestamp = %d, current time = %d\n",
                 strHash, vin.prevout.ToStringShort(), nTimestamp, nNow);
        return false;
    }
    ...
    LogPrintf("CGovernanceManager::GoldminenodeRateCheck -- Rate too high: object hash = %s, goldminenode vin = %s, object timestamp = %d, rate = %f, max rate = %f\n",
...
bool CGovernanceManager::ProcessVote(CNode* pfrom, const CGovernanceVote& vote, CGovernanceException& exception)
{
    ...
    if(mapInvalidVotes.HasKey(nHashVote)) {
        ...
        ostr << "CGovernanceManager::ProcessVote -- Old invalid vote "
                << ", MN outpoint = " << vote.GetVinGoldminenode().prevout.ToStringShort()
    ...
    if(fOk) {
        ...
        if(govobj.GetObjectType() == GOVERNANCE_OBJECT_WATCHDOG) {
            mnodeman.UpdateWatchdogVoteTime(vote.GetVinGoldminenode());
        }
    }
    return fOk;
}

void CGovernanceManager::CheckGoldminenodeOrphanVotes()
{
    LOCK2(cs_main, cs);
    fRateChecksEnabled = false;
    for(object_m_it it = mapObjects.begin(); it != mapObjects.end(); ++it) {
...
void CGovernanceManager::CheckGoldminenodeOrphanObjects()
{
    LOCK2(cs_main, cs);
    int64_t nNow = GetAdjustedTime();
    fRateChecksEnabled = false;
    object_time_m_it it = mapGoldminenodeOrphanObjects.begin();
    while(it != mapGoldminenodeOrphanObjects.end()) {
        object_time_pair_t& pair = it->second;
        CGovernanceObject& govobj = pair.first;

        if(pair.second < nNow) {
            mapGoldminenodeOrphanObjects.erase(it++);
            continue;
        }
        ...
        if(AddGovernanceObject(govobj)) {
            LogPrintf("CGovernanceManager::CheckGoldminenodeOrphanObjects -- %s new\n", govobj.GetHash().ToString());
            govobj.Relay();
            mapGoldminenodeOrphanObjects.erase(it++);
        }
        else {
...
int CGovernanceManager::RequestGovernanceObjectVotes(const std::vector<CNode*>& vNodesCopy)
{
    ...
    // Testnet is ~40 times smaller in goldminenode count, but only ~1000 goldminenodes usually vote,
    // so 1 obj on mainnet == ~10 objs or ~1000 votes on testnet. However we want to test a higher
    // number of votes to make sure it's robust enough, so aim at 2000 votes per goldminenode per request.
    ...
    for (int i = 0; i < nMaxObjRequestsPerNode; ++i) {
        ...
        BOOST_FOREACH(CNode* pnode, vNodesCopy) {
            // Only use reqular peers, don't try to ask from outbound "goldminenode" connections -
            // they stay connected for a short period of time and it's possible that we won't get everything we should.
            // Only use outbound connections - inbound connection could be a "goldminenode" connection
...
int CGovernanceManager::GetGoldminenodeIndex(const CTxIn& goldminenodeVin)
{
    LOCK(cs);
    bool fIndexRebuilt = false;
    int nMNIndex = mnodeman.GetGoldminenodeIndex(goldminenodeVin, fIndexRebuilt);
    if(fIndexRebuilt) {
        RebuildVoteMaps();
        nMNIndex = mnodeman.GetGoldminenodeIndex(goldminenodeVin, fIndexRebuilt);
        if(fIndexRebuilt) {
            LogPrintf("CGovernanceManager::GetGoldminenodeIndex -- WARNING: vote map rebuild failed\n");
        }
    }
    return nMNIndex;
}
...
void CGovernanceManager::RebuildVoteMaps()
{
    ...
    mnodeman.ClearOldGoldminenodeIndex();
}
...
void CGovernanceManager::InitOnLoad()
{
    ...
    LogPrintf("Preparing goldminenode indexes and governance triggers...\n");
    RebuildIndexes();
    AddCachedTriggers();
    LogPrintf("Goldminenode indexes and governance triggers prepared  %dms\n", GetTimeMillis() - nStart);
    LogPrintf("     %s\n", ToString());
}
...
void CGovernanceManager::UpdatedBlockTip(const CBlockIndex *pindex)
{
    ...
    if(!fLiteMode && goldminenodeSync.IsSynced())
        NewBlock();
}
