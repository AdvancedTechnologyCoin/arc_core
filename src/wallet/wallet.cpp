    ...
    for (const auto& outpoint : setWalletUTXO) {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            if(fAnonymizable) {
                ...
                if(fGoldminenodeMode && wtx.tx->vout[i].nValue == 10000*COIN) continue;
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, CAmount& nValueRet, int nPrivateSendRoundsMin, int nPrivateSendRoundsMax) const
{
    ...
    for (const auto& out : vCoins)
    {
        ...
        if(fGoldminenodeMode && out.tx->tx->vout[out.i].nValue == 10000*COIN) continue; //goldminenode input
...
bool CWallet::GetGoldminenodeOutpointAndKeys(COutPoint& outpointRet, CPubKey& pubKeyRet, CKey& keyRet, const std::string& strTxHash, const std::string& strOutputIndex)
{
    ...
    if(vPossibleCoins.empty()) {
        LogPrintf("CWallet::GetGoldminenodeOutpointAndKeys -- Could not locate any valid goldminenode vin\n");
        return false;
    }
    ...
    LogPrintf("CWallet::GetGoldminenodeOutpointAndKeys -- Could not locate specified goldminenode vin\n");
    return false;
}
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-arc-address
                        CScript scriptChange;
