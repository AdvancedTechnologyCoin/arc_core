        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(nCoinType == ONLY_DENOMINATED) {
                    ...
                    found = !(fGoldmineNode && pcoin->vout[i].nValue == 1000*COIN);
                } else if(nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if(found && fGoldmineNode) found = pcoin->vout[i].nValue != 1000*COIN; // do not use Hot MN funds
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, std::vector<COutput>& vCoinsRet, CAmount& nValueRet, int nSpySendRoundsMin, int nSpySendRoundsMax)
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        // goldminenode-like input should not be selected by AvailableCoins now anyway
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
        ...
        for (unsigned int i = 0; i < wtx.vout.size(); i++) {
            ...
            if(fAnonymizable) {
                ...
                if(fGoldmineNode && wtx.vout[i].nValue == 1000*COIN) continue;
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, CAmount& nValueRet, int nSpySendRoundsMin, int nSpySendRoundsMax) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(fGoldmineNode && out.tx->vout[out.i].nValue == 1000*COIN) continue; //goldminenode input
...
bool CWallet::GetGoldminenodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex)
{
    ...
    if(vPossibleCoins.empty()) {
        LogPrintf("CWallet::GetGoldminenodeVinAndKeys -- Could not locate any valid goldminenode vin\n");
        return false;
    }
    ...
    LogPrintf("CWallet::GetGoldminenodeVinAndKeys -- Could not locate specified goldminenode vin\n");
    return false;
}
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-arcticcoin-address
                        CScript scriptChange;
